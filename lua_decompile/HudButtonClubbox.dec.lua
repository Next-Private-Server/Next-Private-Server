...
function HudButtonClubbox:SetVisibility(visible)
  if visible then
...
function HudButtonClubbox:onInit()
...
function HudButtonClubbox:onPostInit()
...
function HudButtonClubbox:hide()
  self:SetVisibility(false)
end
function HudButtonClubbox:show()
  self:SetVisibility(true)
end
function HudButtonClubbox:updateTimer()
  local secsRemaining = game.clubboxShortestTimeAvail()
...
local isPlayerClubboxActive = function()
  local currentClubboxAct = game.player():currentyActiveClubbox()
  return currentClubboxAct and currentClubboxAct:actId() > 0
end
function HudButtonClubbox:onTick(dt)
  self.availabilityTimer = self.availabilityTimer - dt
  if self.availabilityTimer < 0 then
    availabilityTimer = 1
    if game.clubboxActIsAvail() then
      self:updateTimer()
      if isPlayerClubboxActive() then
        if self.showIndicator then
          self.showIndicator = false
          self.Button.Indicator:Hide()
        end
...
function HudButtonClubbox:gotMsgPlayerUpdated(msg)
  self.Button.TicketBar.Text:GetVar("text"):SetString(game.playerClubboxTokens())
end
function HudButtonClubbox:SetClipRect(x, y, w, h)
