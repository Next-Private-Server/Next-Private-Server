...
local MonsterHatchFanfare = {}
local targetMonster, mainElement, colorizeShader, colorizeShaderFactor
local monsterId = 5
local monsterData = game.getMonsterData(monsterId)
local costumeId = 0
local flashSprite, monsterElement, monsterAnim
local viewGroupA = ViewGroup.new()
local mainPanel, eggElement, eggSprite, eggTouch, eggGlow, eggGlowTween, tapParticles, preNotifText
...
local gradientBG, scrollingBG, barTop, barBottom, continueLabel, skipButton
local soundTimer = 0
local tapCounter = 0
local isPostReveal = false
local eggAnimationTimer = 0
local function onEggRefresh(component, element)
  local sporeGfx = monsterData:spore()
  if monsterData:isDipster() then
    if monsterData:isAstralDipster() then
      component:AddRemap("frame", "dipster_door_sheet.xml", "egg_dipster_frame_ASTRAL", false)
      component:AddRemap("door_back", "dipster_door_sheet.xml", "egg_dipster_door_back_ASTRAL", false)
      component:AddRemap("door_shade", "dipster_door_sheet.xml", "egg_dipster_door_shade_ASTRAL", false)
    elseif monsterData:isEpicMonster() then
      component:AddRemap("frame", "dipster_door_sheet.xml", "egg_dipster_frame_EPIC", false)
      component:AddRemap("door_back", "dipster_door_sheet.xml", "egg_dipster_door_back", false)
      component:AddRemap("door_shade", "dipster_door_sheet.xml", "egg_dipster_door_shade", false)
    elseif monsterData:isRareMonster() then
      component:AddRemap("frame", "dipster_door_sheet.xml", "egg_dipster_frame_RARE", false)
      component:AddRemap("door_back", "dipster_door_sheet.xml", "egg_dipster_door_back_RARE", false)
      component:AddRemap("door_shade", "dipster_door_sheet.xml", "egg_dipster_door_shade", false)
    end
    component:AddRemap("egg_dipster_do.png", "gfx/" .. sporeGfx, "", true)
  else
    component:AddRemap("spore_A.png", "gfx/" .. sporeGfx)
  end
end
local eggCrackFrames = {}
eggCrackFrames[1] = "egg_crack_02.png"
eggCrackFrames[2] = "egg_crack_03.png"
eggCrackFrames[3] = "egg_crack_04.png"
eggCrackFrames[4] = "egg_crack_05.png"
eggCrackFrames[5] = "egg_crack_06.png"
eggCrackFrames[6] = "egg_crack_07.png"
local orbCrackFrames = {}
orbCrackFrames[1] = "orb_crack_02"
orbCrackFrames[2] = "orb_crack_03"
orbCrackFrames[3] = "orb_crack_04"
orbCrackFrames[4] = "orb_crack_05"
orbCrackFrames[5] = "orb_crack_06"
orbCrackFrames[6] = "orb_crack_07"
local fugueCrackFrames = {}
fugueCrackFrames[1] = "fugue_crack_02"
fugueCrackFrames[2] = "fugue_crack_03"
fugueCrackFrames[3] = "fugue_crack_04"
fugueCrackFrames[4] = "fugue_crack_05"
fugueCrackFrames[5] = "fugue_crack_06"
fugueCrackFrames[6] = "fugue_crack_07"
local NUM_CRACK_TAPS = 7
local function onEggTap(component, element)
  if tapCounter > 0 and tapCounter < NUM_CRACK_TAPS and monsterData:isDipster() == false then
    if element:parent():V("WasFugued"):GetInt() == 1 then
      game.remapMenuAnim(component, "crack", "fugue_crack_sheet.xml", fugueCrackFrames[tapCounter])
    elseif game.isMagicalNexusIsland() then
      game.remapMenuAnim(component, "crack", "orb_crack_sheet.xml", orbCrackFrames[tapCounter])
    else
      game.remapMenuAnim(component, "crack", "egg_crack_sheet.xml", eggCrackFrames[tapCounter])
    end
  end
  eggSprite("animation"):SetString("tap")
  eggAnimationTimer = 0.5
  lua_sys.playSoundFx("audio/sfx/menu_click_small.wav")
  tapParticles:DoStoredScript("play")
end
local function onTouchedEgg(component, element, x, y)
  tapCounter = tapCounter + 1
  eggSprite:DoStoredScript("Tap")
  if tapCounter == NUM_CRACK_TAPS then
    component("enabled"):SetInt(0)
    eggElement.FadeTransition:Show()
    eggGlowTween:activate()
  end
end
local function loadNextMenu(element)
  local activeIslandType = game.currentIslandType()
  game.setBookOfMonstersIslandId(game.currentIsland(), game.player():getActiveIsland():islandMode())
  game.logEvent("book_o_monsters", "island", tostring(activeIslandType), "source", "hatch_fanfare")
  ...
  top("isCrucibleEvolve"):SetInt(0)
  top.MonsterList.Touch("enabled"):SetInt(0)
  top.MonsterList.Camera("enabled"):SetInt(0)
  if targetMonster then
    targetMonster:mute(false)
  end
  if element:V("WasFugued"):GetInt() == 1 then
    top("WasFugued"):SetInt(1)
  else
...
local function playMonsterSound()
  if soundTimer <= 0 then
    local mId = monsterData:monsterId()
    if monsterData:isModal() then
      local currentMode = game.player():getActiveIsland():islandMode()
      mId = game.getModalMonsterData(monsterData, currentMode):monsterId()
    end
    game.playMonsterSelectSound(mId, 0)
    soundTimer = 1
  end
end
function MonsterHatchFanfare.onInit(element)
  mainElement = element
  ...
  eggElement = element:GetElement("EggImage")
  eggElement.FadeTransition = FadeTransition:new({
    duration = 1,
    maxFade = 1,
    onDoneShow = function()
      flashSprite("alpha"):SetFloat(0)
      flashSprite("visible"):SetInt(1)
      flashSprite.FadeTransition:Show()
      lua_sys.playSoundFx("audio/sfx/monster_level_up.wav")
    end,
    onUpdate = function(alpha)
      eggElement:GetVar("alpha"):SetFloat(alpha)
      colorizeShaderFactor:setFloat(alpha)
    end
  })
  eggElement.FadeTransition:SetAlpha(0)
  eggSprite = eggElement:GetComponent("Sprite")
  eggSprite:addLuaFunction("Refresh", onEggRefresh)
  eggSprite:addLuaFunction("Tap", onEggTap)
  eggSprite:setShader(colorizeShader)
  eggTouch = eggElement:GetComponent("Touch")
  eggTouch:addLuaFunction("onTouchUp", onTouchedEgg)
  eggGlow = eggElement:GetComponent("Glow")
  eggGlowTween = Tweener:new({
    delay = 0.8,
    duration = 0.33,
    targetValue = 20 * game.menuScaleX(),
    onUpdate = function(value)
      eggGlow:setScale(lua_sys.Vector2(value, value))
      eggGlow("alpha"):SetFloat(value / 20)
    end,
    onPostDelay = function()
      eggGlow("visible"):SetInt(1)
    end
  })
  eggGlow("visible"):SetInt(0)
  tapParticles = eggElement:GetComponent("TapParticles")
  ...
  monsterElement.FadeTransition = FadeTransition:new({
    ...
    onDoneShow = function()
      ...
      if element:V("WasFugued"):GetInt() == 1 then
        local structureId = game.FindFugue():uniqueId()
        game.viewedFuguedMonster(structureId)
      else
        game.viewEggInEggHolder()
      end
      continueLabel.FadeTransition:Show()
      isPostReveal = true
    end,
    onUpdate = function(alpha)
      monsterElement:GetVar("alpha"):SetFloat(alpha)
  ...
  skipButton = element:GetElement("SkipButton")
  skipButton:GetComponent("Touch"):addLuaFunction("onTouchUp", function()
    skipButton:DoStoredScript("disable")
    if element:V("WasFugued"):GetInt() == 1 then
      local structureId = game.FindFugue():uniqueId()
      game.viewedFuguedMonster(structureId)
    else
  ...
  viewGroupA:add(eggSprite)
  viewGroupA:add(eggTouch)
  ...
  viewGroupD:hide()
  preNotifText = element:GetElement("PreNotification"):GetComponent("Text")
  element("WasFugued"):SetInt(0)
end
function MonsterHatchFanfare.onPostInit(element)
  monsterAnim("visible"):SetInt(0)
  local duration = 4
...
function MonsterHatchFanfare.onTick(element, dt)
  if soundTimer > 0 then
    soundTimer = soundTimer - dt
  end
  eggElement.FadeTransition:Tick(dt)
  ...
  labelPulser:tick(dt)
  eggGlowTween:tick(dt)
  if eggAnimationTimer > 0 then
    eggAnimationTimer = eggAnimationTimer - dt
    if eggAnimationTimer <= 0 then
      eggSprite("animation"):SetString("idle")
    end
  end
  continueLabel.FadeTransition:Tick(dt)
end
function MonsterHatchFanfare.queuePop(element)
  element:root():popPopUp()
end
function MonsterHatchFanfare.setupMonster(element, monsterPtr)
  if element("WasFugued"):GetInt() == 1 then
    local currentMode = game.player():getActiveIsland():islandMode()
    if currentMode == 0 then
      eggSprite("animationName"):SetString("xml_bin/fugue_crack_MAJ.bin")
      tapParticles("animationName"):SetString("xml_bin/fugue_crack_MAJ.bin")
    else
      eggSprite("animationName"):SetString("xml_bin/fugue_crack_MIN.bin")
      tapParticles("animationName"):SetString("xml_bin/fugue_crack_MIN.bin")
    end
    preNotifText("text"):SetString("FUGUE_FANFARE_TAP")
  end
  targetMonster = monsterPtr
  monsterId = element("MonsterID"):GetInt()
  monsterData = game.getMonsterData(monsterId)
  costumeId = element("CostumeID"):GetInt()
  if monsterData:isDipster() then
    eggSprite("animationName"):SetString("xml_bin/dipster_door.bin")
    tapParticles("animationName"):SetString("xml_bin/dipster_door.bin")
    preNotifText("text"):SetString("DIPSTER_FANFARE_TAP")
  end
  eggSprite:DoStoredScript("Refresh")
  if monsterData:isModal() then
    local currentMode = game.player():getActiveIsland():islandMode()
    local animFile = game.getModalMonsterData(monsterData, currentMode):animationFile()
    monsterAnim("animationName"):SetString("xml_bin/" .. animFile)
  else
    monsterAnim("animationName"):SetString("xml_bin/" .. monsterData:animationFile())
  end
  if monsterData:isBoxMonster() or monsterData:isTitansoul() then
    if monsterData:isEpicMonster() and game.onGoldIsland() then
      mainPanel.OffsetTransition:Cancel()
      mainPanel.OffsetTransition:SetOffset(mainPanel.endX, mainPanel.endY)
      local entityId = monsterData:entityId()
      if entityId == 1525 then
        monsterAnim("animation"):SetString("01-F_EPIC_Dance_Plant_01")
      elseif entityId == 1526 then
        monsterAnim("animation"):SetString("01-F_EPIC_Dance_Cold_01")
      elseif entityId == 1527 then
        monsterAnim("animation"):SetString("01-F_EPIC_Dance_Air_01")
      elseif entityId == 1528 then
        monsterAnim("animation"):SetString("01-F_EPIC_Dance_Water_01")
      elseif entityId == 1529 then
        monsterAnim("animation"):SetString("01-F_EPIC_Dance_Earth_01")
      else
        monsterAnim("animation"):SetString("01-F_EPIC_Dance_Default_01")
      end
    else
      monsterAnim("animation"):SetString("Idle")
    end
    local scale = math.min(game.hudScale(), 100 * game.menuScaleX() / monsterData:height())
  ...
  elseif monsterData:isDipster() then
  ...
  if monsterData:isAdult() and monsterData:isCelestial() then
    postNotificationTop("text"):SetString(LOC("NOTIFICATION_ASCENDED_MONSTER"))
  end
  if monsterData:isWubbox() and monsterData:isEpicMonster() and game.onGoldIsland() then
    postNotificationTop("text"):SetString(LOC("NOTIFICATION_POWERED_UP_MONSTER"))
  end
  if game.isMagicalNexusIsland() then
    element:GetElement("PreNotification"):GetComponent("Text")("text"):SetString(LOC("TRANSPOSE_FANFARE_TAP"))
    postNotificationTop("text"):SetString(LOC("TRANSPOSE_FANFARE_RESULTS_HEADER"))
  end
  if monsterData:isTitansoul() then
    postNotificationTop("text"):SetString(LOC("HATCH_FANFARE_RESULTS_HEADER_TITANSOUL"))
  end
  local gradient, pattern
  if monsterData:isSeasonal() and game.monsterFanfareBg ~= nil and game.monsterFanfareBg(monsterId) ~= "" then
  ...
  elseif monsterData:isTitansoul() then
    gradient = "gfx/menu/gradient_bg_titansoul"
    pattern = "gfx/menu/bg_symbols_titansoul"
  elseif monsterData:isDipster() then
    gradient = "gfx/menu/gradient_bg_dipster"
    pattern = "gfx/menu/bg_symbols_dipster"
  ...
  if monsterData:isTitansoul() or monsterData:isBoxMonster() and not game.isAmberIsland() or game.isUnderlingIsland() or game.isCelestialIsland() and not monsterData:isDipster() or game.onGoldIsland() then
    viewGroupA:hide()
    eggTouch("enabled"):SetInt(0)
    if monsterData:isWubbox() and monsterData:isEpicMonster() and game.onGoldIsland() then
      flashSprite("alpha"):SetFloat(0)
      flashSprite("visible"):SetInt(1)
      flashSprite.FadeTransition:Show()
