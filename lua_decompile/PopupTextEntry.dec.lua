...
function PopupTextEntry:new(obj)
  local obj = obj or {}
  setmetatable(obj, self)
  self.__index = self
  return obj
end
function PopupTextEntry:onInit()
...
function PopupTextEntry:onPostInit()
...
function PopupTextEntry:setup(entryID, titleText, descText, defaultText, maxChars)
...
function PopupTextEntry:onTick(dt)
  if self.transitionState ~= 0 then
    ...
    elseif 0 >= self.transitionTime then
      if self.choice == "true" then
        self:root():popPopUp()
        game.submitTextEntry(self.messageID, self.text, true)
      else
        self:root():popPopUp()
        game.submitTextEntry(self.messageID, self.text, false)
      end
    end
  end
...
function PopupTextEntry:TickTransition()
...
function PopupTextEntry:queuePop()
  self.transitionState = 2
end
function PopupTextEntry:gotMsgKeyDown(msg)
  if self.activated then
...
function PopupTextEntry:gotMsgKeyUp(msg)
  if self.activated then
    self.lastKey = 0
    self.lastModKey = 0
  end
end
function PopupTextEntry:gotMsgKeyRepeat(msg)
  if self.activated then
    self.lastKey = msg.key.val
    self.lastModKey = msg.modKey.val
    self:doKeyActions()
...
function PopupTextEntry:gotMsgKeyChar(msg)
  if self.activated then
...
function PopupTextEntry:doKeyActions()
  if self.activated then
...
function PopupTextEntry:adjustCursorPosition()
