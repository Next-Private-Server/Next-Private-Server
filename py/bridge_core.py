...
try:
    ...
    _debug_logger.exception(
        "Could not open the diagnostic log file; in-app logging remains enabled"
    )
...
def set_nps_online_config(
    ...
    nps_online.set_nps_online_config(
        server_url, account_id, session_token, device_id, friend_code, cert_hash
    )
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=list(SETTINGS.get("cors_origins") or ["*"]),
    allow_credentials=bool(SETTINGS.get("cors_credentials", False)),
    allow_methods=["*"],
    allow_headers=["*"],
)
DEFAULT_ACCOUNT = {
    "username": "Next Private Server",
    "email": "Nextstars@gmail.com",
    "password": "PrivateServerStudios",
    "user_id": "00000001AB",
    "user_game_id": "NextPrivateServer",
...
BLUEBOX_HTTP_PORT = int(
    _HTTP_PORTS_SETTING[1] if len(_HTTP_PORTS_SETTING) > 1 else _HTTP_PORTS_SETTING[0]
)
...
def aes_encrypt(value):
    ...
    return base64.b64encode(
        _a.encrypt(pad(value.encode("utf-8"), AES.block_size))
    ).decode("utf-8")
...
def load_whitelist():
    ...
    return (
        set((str(item).strip() for item in _a if str(item).strip()))
...
def token(username, user_game_id, login_type, account_id_value, game_id, ttl=None):
    ...
    _b = {
        ...
        "token_version": 1,
        "time_created": _a,
        "expires_at": _a + int(ttl or SETTINGS.get("token_ttl", 900)),
...
async def gate(request, call_next):
    ...
    if _e != _d:
        logger.info(
            "%s %s -> %s %s ip=%s auth=%s ua=%r referer=%r query=%r",
            request.method,
            _e,
            _d,
            _h.status_code,
        ...
        logger.info(
            "%s %s %s ip=%s auth=%s ua=%r referer=%r query=%r",
            request.method,
            _d,
            _h.status_code,
            _c,
...
def sfs_block():
    ...
    return {
        ...
        "hostname": _a,
        "serverAddress": _a,
        "serverId": int(SETTINGS.get("server_id", 1)),
        "server_id": int(SETTINGS.get("server_id", 1)),
        "serverIp": _i,
        "serverIP": _i,
        "server_ip": _i,
        "serverHost": _a,
...
def pregame_response_exact():
    ...
    return {
        "ok": True,
        "serverId": server_id(),
        "serverIp": sfs_block()["serverIp"],
        "contentUrl": content_root(),
    }
...
def game_config_response_exact():
    ...
    return {
        ...
        "config": {
            ...
            "type": "android",
            "age_gate_consent_required": bool(
                SETTINGS.get("age_gate_consent_required", True)
            ),
            "gdpr_consent_required": bool(SETTINGS.get("gdpr_consent_required", False)),
        },
        "geo": str(SETTINGS.get("geo", "US")),
        "gag": int(SETTINGS.get("gag", 13)),
    }
...
def token_response_exact(account, ttl=None):
    ...
    _access_token = token(
        ...
        _user_game_id,
        "email",
        account_id(account.get("user_id"), 1),
        SETTINGS.get("game_id", "msm"),
        ttl,
    )
    return {
        "ok": True,
        "user_game_id": [_user_game_id],
        "login_types": ["email"],
        "access_token": _access_token,
        "token_type": "bearer",
        "expires_at": round(time.time()) + int(ttl or SETTINGS.get("auth_ttl", 1200)),
        "device_updated": True,
    }
...
def email_account_response_exact(
    username, password, account_id_value, user_game_id_value, ttl=None
):
    ...
    return {
        ...
        "password": password,
        "account_id": account_id_value,
        "user_game_id": user_game_id_value,
        "login_type": "email",
        "time_created": _now,
        "access_token": _access_token,
        "token_type": "bearer",
        "expires_at": _now + int(ttl or SETTINGS.get("auth_ttl", 1200)),
        "device_updated": True,
    }
...
async def email_account_handler(request: Request):
    ...
    try:
        _d = await params(request)
        logger.info(
            "email_account_handler path=%s params=%s",
            request.scope.get("path"),
            sorted(_d.keys()),
        )
        _username = str(
            _d.get("username") or _d.get("email") or _d.get("u") or ""
        ).strip()
        ...
        if find_account(_username, _password) or any(
            (str(_a.get("username", "")) == _username for _a in load_accounts())
        ):
        ...
        _account = {
            ...
            "password": _password,
            "user_id": short_id(),
            "user_game_id": short_id(),
        }
        _accounts = load_accounts()
        _accounts.append(_account)
        save_json(ACCOUNTS_PATH, _accounts)
        _c = email_account_response_exact(
            _username,
            _password,
            _account["user_id"],
            _account["user_game_id"],
            SETTINGS.get("auth_ttl", 1200),
        )
        _r = JSONResponse(_c)
...
async def find_account_handler(request: Request):
    ...
    logger.info(
        "find_account path=%s params=%s", request.scope.get("path"), sorted(_b.keys())
    )
...
async def waf_handler(request: Request):
    ...
    logger.info(
        "waf path=%s method=%s query=%s body=%r",
        request.scope.get("path"),
        request.method,
...
_NPS_SERVER_CONFIG_PATH = Path(
    "/storage/emulated/0/Android/data/com.nextstars.nps/files/nps_server.properties"
)
...
async def nps_server_config(request: Request):
    ...
    return Response(
        "Server configuration is not ready", status_code=503, media_type="text/plain"
    )
...
async def game_config(request: Request):
    _b = await params(request)
    logger.info(
        "game_config path=%s params=%s", request.scope.get("path"), sorted(_b.keys())
    )
...
def _build_downloads_xml(entries):
    ...
    lines = [
        '<?xml version="1.0"?>',
        f'<Downloads version="{CLIENT_VERSION }" build="{CLIENT_BUILD }">',
    ]
    for entry in entries:
        ...
        if not name:
            continue
        lines.append(
            f'    <Download file="{name }" checksum="{entry ["checksum"]}" major="{_major }" minor="{_minor }" micro="{_micro }" rev="0" />'
        )
...
async def files_manifest(request: Request):
    ...
    entries = manifest()
    logger.info(
        "files_manifest path=%s params=%s entries=%d",
        request.scope.get("path"),
        sorted(_a.keys()),
        len(entries),
    )
...
async def downloads_xml(request: Request):
    entries = manifest()
    logger.info(
        "downloads_xml path=%s entries=%d", request.scope.get("path"), len(entries)
    )
...
async def catch_all(path: str, request: Request):
    _a = await request.body()
    logger.info(
        "catch_all path=%s method=%s query=%s body=%r",
        path,
        request.method,
        str(request.url.query),
        _a[:2000],
...
async def bluebox(request: Request):
    logger.info(
        "bluebox probe path=%s method=%s", request.scope.get("path"), request.method
    )
    return Response(
...
def _safe_error_frame(command, message):
    return (
        command,
        msm_protocol.build_raw_frame(
            command, {"success": False, "error": message, "notificationOnFail": False}
        ),
    )
    ...
    with _PROCESS_LOCK:
        try:
            ...
            _results = [
                (
                    command,
                    {"success": False, "error": str(_err), "notificationOnFail": False},
                )
            ]
...
async def sfs_websocket(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            try:
                _encoded = await asyncio.to_thread(
                    _process_frame, _frame.command, _frame.params
                )
            except Exception as _err:
...
app.add_api_route(
    f"/{FILES_DIR .name }/files.json", files_manifest, methods=["GET", "POST"]
)
app.add_api_route(
    f"/{FILES_DIR .name }/downloads.xml", downloads_xml, methods=["GET", "POST"]
)
app.add_api_route(f"/{FILES_DIR .name }/{{path:path}}", serve_file, methods=["GET"])
app.add_api_route(
    PUBLIC_CONTENT_PREFIX.rstrip("/"), files_manifest, methods=["GET", "POST"]
)
app.add_api_route(PUBLIC_CONTENT_PREFIX, files_manifest, methods=["GET", "POST"])
app.add_api_route(
    f"{PUBLIC_CONTENT_PREFIX }files.json", files_manifest, methods=["GET", "POST"]
)
app.add_api_route(
    f"{PUBLIC_CONTENT_PREFIX }downloads.xml", downloads_xml, methods=["GET", "POST"]
)
...
async def _serve_with_retry(host, port, log_level, max_attempts=10, delay=1.0):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if _SHUTDOWN_REQUESTED or attempt >= max_attempts:
            raise last_exc
        logger.warning(
            "port %s bind failed (attempt %s/%s): %s; retrying",
            port,
            attempt,
            max_attempts,
            last_exc,
...
async def main():
    ...
    logger.info(
        "Runtime ready: base=%s, players=%s, functioning_currencies=%s",
        BASE_DIR,
        msm_store.players_dir,
        msm_toggles.is_enabled("functioning_currencies"),
    )
