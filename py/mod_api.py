...
from mod_schema import (
    ModValidationError,
    validate_island_entry,
    validate_monster_catalog_entry,
)
...
_CURRENCY_ALIASES = {
    ...
    "eth_currency": "ethereal_currency",
    "key": "keys",
    "relic": "relics",
    "star": "starpower",
    "stars": "starpower",
    "wildcard": "egg_wildcards",
    "wildcards": "egg_wildcards",
    "eggwildcards": "egg_wildcards",
    "clubbox_token": "clubbox_tokens",
    "clubbox_tokens": "clubbox_tokens",
}
...
def _dispatch_hud_prompt(username="", command="", params=None, **_):
    if (
        command != "gs_set_displayname"
        or not isinstance(params, dict)
        or not _HUD_PROMPT_HANDLERS
    ):
        return
...
def add_currency(player_object, currency, amount):
    ...
    if currency not in _CURRENCY_ACTUAL_KEYS:
        ...
        return
    player_object[currency] = _clamp_client_int(
        int(player_object.get(currency, 0) or 0) + _safe_int(amount, 0)
    )
...
def add_new_structure(
    ...
    **overrides,
    ...
    _apply_db_patch(
        "db_structure",
        {"structures_data": [entry]},
        "inject",
        "<%s:add_new_structure:%s>" % (mod_id, key),
    )
    logger.info(
        "%s registered structure '%s' as id %d via add_new_structure",
        mod_id,
        key,
        structure_id,
    )
...
def _add_monster_to_island_book(monster_id, mod_id, key, island_type, genes):
    ...
    island = next(
        (
            isl
            for isl in islands.get("islands_data", [])
            if isinstance(isl, dict) and isl.get("island_type") == island_type
        ),
        None,
    )
    if island is None:
        logger.warning(
            "add_new_monster(add_to_island=...): island_type %s not found", island_type
        )
        return
    monsters = island.get("monsters")
    if not isinstance(monsters, list) or not monsters:
        logger.warning(
            "add_new_monster(add_to_island=...): island_type %s has no book slots",
            island_type,
        )
        return
...
def add_new_monster(
    key, common_name, genes, graphic_file, add_to_island=False, **overrides
):
    mod_id = _caller_mod_id()
    ...
    _apply_db_patch(
        ...
        {"monsters_data": [entry]},
        "inject",
        "<%s:add_new_monster:%s>" % (mod_id, key),
    )
    logger.info(
        "%s registered monster '%s' as id %d via add_new_monster",
        mod_id,
        key,
        monster_id,
    )
    if add_to_island:
        island_type = (
            add_to_island
            if isinstance(add_to_island, int) and not isinstance(add_to_island, bool)
            else _PLANT_ISLAND_TYPE
        )
        _add_monster_to_island_book(
            monster_id, mod_id, key, island_type, entry.get("genes")
        )
    return monster_id
...
def _event_payload(event_name, kwargs):
    ...
    payload = {
        "event": event_name,
        "id": str(
            _first_value(kwargs.get("id"), kwargs.get("request_id"), command) or ""
        ),
        "username": username,
        "command": command,
    ...
    for key, value in kwargs.items():
        ...
        if key == "params" and isinstance(value, dict):
            ...
            for param_key, param_value in value.items():
                if param_key not in payload and (
    ...
    if (
        payload.get("user_monster_id")
        and not payload.get("user_egg_id")
        and command in ("gs_hatch_egg", "gs_finish_breeding")
    ):
...
class LauncherTab:
    ...
    def text(self, value):
        self.elements.append(
            {"type": "text", "id": self._new_id(), "text": str(value)[:2000]}
        )
    ...
    def image(self, source):
        self.elements.append(
            {"type": "image", "id": self._new_id(), "source": str(source)[:2_000_000]}
        )

    def upload_file(self, label, on_upload=None, accept=None):
        cb = self._register_callback(on_upload)
        safe_accept = (
            [str(a) for a in accept] if isinstance(accept, (list, tuple)) else []
        )
        self.elements.append(
            {
                "type": "fileUpload",
...
def open_launcher_tab_json(mod_id, tab_id, username):
    ...
    tab = (
        _LAUNCHER_TABS.get(safe_mod_id, {}).get(safe_tab_id)
        if safe_mod_id and safe_tab_id
        else None
    )
    if tab is None:
    ...
    if tab._on_open:
        try:
            ...
            logger.exception(
                "launcher tab %s:%s on_open failed", safe_mod_id, safe_tab_id
            )
...
def fire_launcher_callback_json(mod_id, tab_id, callback_id, username, value_json):
    ...
    if tab is None:
        return json.dumps({"error": "unknown tab"})
    if not isinstance(callback_id, str) or not callback_id.startswith(
        f"{safe_mod_id }:"
    ):
        logger.warning(
            "blocked cross-mod launcher callback %r for mod %s",
            callback_id,
            safe_mod_id,
        )
...
def dispatch_action(action_name, username="", params=None):
    ...
    try:
        logger.info(
            "dispatching java mod action %s params=%s", safe_name, payload["params"]
        )
        raw_commands = runtime.callAction(
            safe_name, json.dumps(payload, ensure_ascii=False)
        )
        commands = json.loads(str(raw_commands or "[]"))
        logger.info(
            "java mod action %s returned %d command(s)",
            safe_name,
    ...
    for command in commands or []:
        ...
        if command_type == "send_frame":
            ...
            if frame_command:
                ...
                logger.warning(
                    "blocked invalid send_frame command %r", command.get("command")
                )
            continue
        if command_type == "client_request":
            ...
            if frame_command:
                logger.info(
                    "java client request queued: %s params=%s", frame_command, payload
                )
                _PENDING_FRAMES.setdefault(username, []).append(
                    (frame_command, payload)
                )
                ...
                logger.warning(
                    "blocked invalid client_request command %r", command.get("command")
                )
            continue
        if command_type == "run_server_handler":
            ...
            if frame_command:
                try:
                    ...
                    logger.info(
                        "java mod requested server handler %s params=%s",
                        frame_command,
                        payload,
                    )
                    _PENDING_FRAMES.setdefault(username, []).extend(
                        msm_handlers.handle_command(frame_command, payload)
                    )
                except Exception:
                    logger.exception(
                        "server handler command failed for %s", frame_command
                    )
            else:
                logger.warning(
                    "blocked invalid run_server_handler command %r",
                    command.get("command"),
                )
            continue
...
def register_monster(monster_id, definition):
    ...
    if monster_id in catalog:
        raise ModValidationError(
            f"monster_id {monster_id } already exists in the catalog"
        )
...
def _load_staged_server_scripts(mod_id, mod_dir):
    ...
    if os.path.isdir(lua_dir):
        lua_files = [
            entry
            for entry in sorted(os.listdir(lua_dir))
            if entry.lower().endswith(".lua")
        ]
        if lua_files:
            logger.warning(
                "server lua for %s is staged but not executed: no embedded server lua runtime",
                mod_id,
            )
...
def _apply_assets_file(asset_path):
    ...
    if isinstance(aliases, dict):
        for source, target in aliases.items():
            ...
            if safe_source and safe_target:
                ...
                logger.info(
                    "mod asset command alias %s -> %s", safe_source, safe_target
                )
            else:
                logger.warning(
                    "asset %s blocked invalid command alias %r -> %r",
                    asset_path,
                    source,
                    target,
                )
    ...
    for raw_name, patch_spec in db_patches.items():
        ...
        if not _safe_command_name(db_name):
            ...
            continue
        if isinstance(patch_spec, dict) and (
            "data" in patch_spec or "mode" in patch_spec
        ):
...
def _apply_strings(strings, asset_path, locale=""):
    ...
    for db_name in (
        ...
        injected = True
    logger.info(
        "mod asset loaded %d string(s)%s from %s%s",
        len(safe_strings),
...
def _apply_db_patch(db_name, patch_data, mode, asset_path):
    ...
    if not isinstance(patch_data, (dict, list)):
        logger.warning(
            "asset %s ignored %s: patch data must be object or array",
            asset_path,
            db_name,
        )
        return
    ...
    if current is None:
        logger.warning(
            "asset %s cannot inject %s: db file does not exist", asset_path, db_name
        )
        return
            ...
            if key:
                existing = next(
                    (
                        entry
                        for entry in target
                        if isinstance(entry, dict) and entry.get(key) == item.get(key)
                    ),
                    None,
                )
