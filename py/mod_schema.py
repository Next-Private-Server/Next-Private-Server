...
def validate_monster_catalog_entry (definition ):
    ...
    if missing :
        raise ModValidationError (f"monster catalog entry missing required fields: {missing }")
    if not isinstance (definition .get ("graphic"),dict )or not definition ["graphic"].get ("file"):
        raise ModValidationError ("monster catalog entry needs graphic.file pointing at an existing shipped asset key")
