...
def _requested_gene(params, key="end_gene"):
    gene = (
        params.get(key)
        or (params.get("gene") if key == "end_gene" else None)
        or params.get("genes")
        or ""
    )
...
def _grant_attuned_critter(island, gene):
    ...
    if match is None:
        ...
        num = (
            max(
                match.get("num", 0) or 0,
                match.get("count", 0) or 0,
                match.get("amount", 0) or 0,
            )
            + 1
        )
        match.clear()
...
def start_attuning(username, params):
    ...
    attuning_data = {
        "reattuned_monster": SFSLong(reattuned_monster),
    ...
    island["attuning"] = [
        e for e in entries if e is not None and e.get("structure") != structure_id
    ]
        ...
        structure.update(
            {
                ...
                "building_completed": complete_on,
                "finishing_time": complete_on,
                "finished_at": complete_on,
                "seconds_remaining": _ATTUNING_DURATION_MS // 1000,
                "time_remaining": _ATTUNING_DURATION_MS,
                "end_gene": end_gene,
                "genes": end_gene,
    ...
    if island is not None:
        ...
        if end_gene and reattuned_monster:
            ...
            if tuned is not None:
                tuned["reattuned_genes"] = (
                    tuned.get("reattuned_genes") or ""
                ) + end_gene
                tuned_update = {
                    "user_monster_id": SFSLong(reattuned_monster),
                    "reattuned_genes": tuned["reattuned_genes"],
                }
        if structure is not None:
            structure.update(
                {
...
def update_reattune_monster(username, params):
    ...
    reattuning_data = {
        "structure": SFSLong(structure_id),
        "monster": SFSLong(user_monster_id),
    }
...
def collect_reattune_monster(username, params):
    ...
    if island is not None:
        ...
        if entry is not None:
            user_monster_id = int(
                entry.get("monster", entry.get("user_monster_id", 0)) or 0
            )
        ...
        now = int(time.time() * 1000)
        attuning_data = {
            "reattuned_monster": SFSLong(0),
            "started_on": SFSLong(now),
            "start_gene": "",
            "complete_on": SFSLong(now + _ATTUNING_DURATION_MS),
            "end_gene": "G",
            "structure": SFSLong(structure_id),
        }
    ...
    return {
        ...
        "new_monst": new_monster,
        "user_attuning_data": attuning_data,
        "user_reattuning_data": {
            "structure": SFSLong(structure_id),
            "monster": SFSLong(0),
        },
...
def _apply_tune_up(monster, rarity):
    ...
    evolves_into = (msm_gamedata.get_monster_definition(species_id) or {}).get(
        "evolve_into"
    ) or 0
    next_id = (
        msm_gamedata.get_monster_id_for_entity_id(evolves_into) if evolves_into else 0
    )
    if not next_id:
    ...
    monster["name"] = (
        next_definition.get("common_name")
        or next_definition.get("name")
        or monster.get("name", "")
    )
    return True
...
def viewed_reattuned_monster(username, params):
    ...
    if island is not None and monster is not None:
        result["sold_monsters_update"] = _record_new_species(
            island, monster.get("monster", 0) or 0
        )
        save_player(username, root)
    return result
