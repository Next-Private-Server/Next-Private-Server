...
def is_box_monster_entity(definition):
    return (
        bool(definition)
        and (definition.get("entity_type") or "").lower() == "box_monster"
    )
...
def _is_rare_monster_definition(definition):
    ...
    return (
        "CLASS_RARE" in class_name
        or common_name.startswith("rare ")
        or name.endswith("_rare")
    )
...
def round2_wire_progress(monster):
    ...
    if not flex_progress:
        ...
        for flexegg_id in flex_requirements:
            if flexegg_id in remaining and _count(remaining, flexegg_id) > _count(
                exact_requirements, flexegg_id
            ):
...
def evolve_requirements_combined(monster):
    return _ints_from_raw(monster.get("has_evolve_reqs")) + _ints_from_raw(
        monster.get("has_evolve_flexeggs")
    )
...
def append_boxed_egg(monster, boxed_monster_id):
    ...
    if "box_requirements" in monster:
        ...
        requirements = box_requirements(
            get_monster_definition(monster.get("monster", 0)), 0
        )
...
def repair_underling_box_state(island):
    ...
    for monster in island.get("monsters") or []:
        ...
        if bool(monster.get("awakened")):
            if (
                island_type in UNDERLING_ISLAND_TYPES
                and (definition or {}).get("evolve_into", 0)
                and not monster.get("evolve_unlocked")
            ):
                monster["evolve_unlocked"] = 1
            if (
                island_type in UNDERLING_ISLAND_TYPES
                and monster.get("ascend_pending")
            ...
            if (
                island_type in UNDERLING_ISLAND_TYPES
                and (definition or {}).get("evolve_into", 0)
                and not monster.get("ascend_pending")
            ):
                ...
                if evolve_reqs or evolve_flexeggs:
                    ...
                    monster["has_evolve_flexeggs"] = _ints_to_json_array(
                        evolve_flexeggs
                    )
...
def start_amber_vessel_timer_if_needed(monster, island_type):
    if (
        island_type != 22
        or not boxed_eggs(monster)
        or (monster.get("egg_timer_start", 0) or 0) > 0
    ):
        return
    import time
...
def build_placeholder_box_monster(
    monster_id,
    definition,
    island_uid,
    user_monster_id,
    ...
    if not force_locked and (
        is_amber_no_vessel_island(island_type)
        or msm_toggles.is_enabled("wubbox_auto_awaken")
    ):
...
def _find_box_island(player_object, params):
    ...
    if island_id:
        island = find_island(
            player_object, island_id if island_id >= 1000 else island_id + 1000
        )
...
def _box_progress_update(monster):
    progress = (
        evolve_boxed_eggs(monster)
        if monster.get("ascend_pending")
        else boxed_eggs(monster)
    )
    return {
        "user_monster_id": SFSLong(monster.get("user_monster_id", 0)),
        "egg_timer_start": SFSLong(
            monster.get("egg_timer_start", -1)
            if monster.get("egg_timer_start") is not None
            else -1
        ),
...
def box_add_egg(username, params):
    ...
    target_definition = (
        get_monster_definition(target_box.get("monster", 0))
    ...
    if egg is not None:
        ...
        breeding_island, breeding_record = msm_monsters._find_breeding_record(
            player_object, user_egg_id
        )
        if breeding_record is not None:
            ...
            if not egg_type:
                egg_type = msm_monsters.choose_breeding_result_monster(
                    breeding_record.get("monster_1", 0),
                    breeding_record.get("monster_2", 0),
                )
    ...
    if (
        target_box.get("ascend_pending")
        and _evolve_requirement_slot_for_monster(target_box, egg_type) is None
    ):
...
def box_add_monster(username, params):
    ...
    if target_box.get("ascend_pending"):
        ...
        if _count(target_requirements, boxed_monster_type) <= _count(
            target_current, boxed_monster_type
        ):
            result["error"] = "monster_not_required"
            result["message"] = "NOTIFICATION_MONSTER_NOT_REQUIRED"
            return result
    else:
        target_requirements = box_requirements(
            get_monster_definition(target_box.get("monster", 0)), island_type_of(island)
        )
        target_current = boxed_eggs(target_box)
        if _count(target_requirements, boxed_monster_type) <= _count(
            target_current, boxed_monster_type
    ...
    if island_type in BOX_SIMPLE_ISLAND_TYPES:
        ...
        target_box["numEggsInInventory"] = max(
            len(boxed_eggs(target_box)), target_box.get("numEggsInInventory", 0) or 0
        )
    ...
    return {
        "user_box_monster_id": SFSLong(requested_box_id),
        "success": True,
        "user_monster_id": SFSLong(user_monster_id),
        "monster_type": boxed_monster_type,
    }
...
def box_purchase_fill(username, params):
    ...
    target_island, box_monster = find_monster_with_island(
        player_object, user_monster_id, island
    )
    ...
    if not is_box_monster_entity(definition) and not box_monster.get("ascend_pending"):
        ...
        is_awakened_box_monster(box_monster)
        and can_evolve
        and not box_monster.get("ascend_pending")
    ):
        _start_evolve_round(box_monster, definition, island_type_of(island))
    ...
    if filled > 0:
        egg_wildcards = max(
            player_object.get("egg_wildcards", 0) or 0,
            player_object.get("playerEggWildcards", 0) or 0,
        )
        remaining = filled
        ...
        if remaining > 0:
            ...
            if island_type_of(island) == GOLD_ISLAND_TYPE:
                ...
                per_monster_key = (
                    "USER_GOLD_EPIC_BOX_INVENTORY_DIAMOND_PRICE_PER_MONSTER"
                    if "EPIC" in class_name
                    else (
                        "USER_GOLD_RARE_BOX_INVENTORY_DIAMOND_PRICE_PER_MONSTER"
                        if "RARE" in class_name
    ...
    result = {
        "success": True,
        "user_monster_id": SFSLong(user_monster_id),
        "user_box_monster_id": SFSLong(user_monster_id),
        "gi_monster_id": SFSLong(user_monster_id),
        "properties": create_player_properties(player_object),
    }
    if round2:
...
def wake_wubbox(username, params):
    ...
    monster = None
    if user_monster_id:
        found_island, monster = find_monster_with_island(
            player_object, user_monster_id, island
        )
    ...
    if monster is None:
        for m in island.get("monsters") or []:
            if (
                m is not None
                and not is_awakened_box_monster(m)
                and is_box_monster_entity(get_monster_definition(m.get("monster", 0)))
            ):
                monster = m
    ...
    logger.info(
        ...
        monster.get("box_requirements"),
        is_awakened_box_monster(monster),
    )
    if is_awakened_box_monster(monster):
        ...
        logger.info(
            ...
            monster.get("monster"),
            evolve_into_entity_id,
            next_monster_id,
        )
        if monster.get("ascend_pending"):
            round2_required = evolve_requirements_combined(monster)
            if round2_required and len(evolve_boxed_eggs(monster)) < len(
                round2_required
            ):
                logger.info(
        ...
        if next_monster_id > 0 and monster.get("monster") != next_monster_id:
            ...
            if further_evolve_into > 0:
                ...
                further_evolve_reqs = _ints_from_raw(
                    new_definition.get("evolve_requirements")
                )
                further_evolve_flexeggs = _ints_from_raw(
                    new_definition.get("evolve_req_flexeggs")
                )
                monster["has_evolve_reqs"] = _ints_to_json_array(further_evolve_reqs)
                monster["has_evolve_flexeggs"] = _ints_to_json_array(
                    further_evolve_flexeggs
                )
                monster["evolve_boxed_eggs"] = "[]"
                monster["evolve_flex_boxed_eggs"] = "[]"
                monster["ascend_pending"] = True
            ...
            client_result = {
                "success": True,
                "user_monster_id": SFSLong(user_monster_id),
                "monster": next_monster_id,
                "evolution": True,
            }
            update = _box_activate_update(monster, species_changed=True)
            logger.info(
                "wake_wubbox species swap response client_result=%r update=%r",
                client_result,
                update,
            )
        ...
        logger.info(
            "wake_wubbox no-op (no evolve_into) response client_result=%r update=%r",
            client_result,
            update,
        )
        return client_result, update
    ...
    if island_type == 22:
        ...
        for i in range(len(monsters) - 1, -1, -1):
            if (
                ...
                and monsters[i].get("user_monster_id") == user_monster_id
            ):
                del monsters[i]
                break
        sell_result = {
        ...
        user_egg, nursery = msm_monsters.place_egg(
            ...
            vessel_species,
            "wake_wubbox",
            previous_name=vessel_name or None,
        )
        if user_egg is None:
            save_player(username, root)
...
def purchase_evolve_unlock(username, params):
    ...
    _start_evolve_round(
        monster, definition, island_type_of(unlock_island) if unlock_island else 0
    )
    save_player(username, root)
    result = {
        "success": True,
