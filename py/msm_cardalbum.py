...
def _cards():
    if "cards" not in _cache:
        ...
        _cache["cards"] = {
            c["id"]: c for c in (data.get("card_data") or []) if "id" in c
        }
    return _cache["cards"]
    ...
    if "albums" not in _cache:
        ...
        for album in data.get("card_album_data") or []:
            pages = [
                {
                    "id": p.get("id"),
                    "cards": _json_list(p.get("cards")),
                    "rewards": _json_list(p.get("rewards")),
                }
    ...
    if "store_items" not in _cache:
        ...
        _cache["store_items"] = {
            i["id"]: {
                ...
                "contents": _json_list(i.get("contents")),
            }
            for i in data.get("card_album_store_item_data") or []
            if "id" in i
        }
...
def _cards_for_album(album_id):
    ...
    if album_id not in per_album:
        wanted_ids = {
            card_id for page in _albums().get(album_id, []) for card_id in page["cards"]
        }
        all_cards = _cards()
    ...
    for entry in player_object.get("card_albums") or []:
        if entry.get("card_album_id") == album_id:
            ...
            return {
                c["i"]
                for c in cards
                if isinstance(c, dict) and (c.get("n", 0) or 0) > 0
            }
    return set()
...
def _grant_cards(player_object, card_ids):
    for card_id in card_ids:
        cards = (
            _album_entry(player_object, _card_album_id(card_id))
            .setdefault("data", {})
            .setdefault("cards", [])
        )
...
def _target_album_id(player_object):
    return (
        active_album_id() or player_object.get("last_card_album") or _current_album_id()
    )
...
def _sync_sticker_star_currency(player_object, album_id=None):
    ...
    legacy_values = [
        _safe_int(player_object.get(key))
        for key in ("sticker_stars", "sticker_stars_actual")
        if key in player_object
    ]
    for existing in player_object.get("card_albums") or []:
        if isinstance(existing, dict):
            legacy_values.append(
                _safe_int((existing.get("data") or {}).get("currency"))
            )
    if legacy_values:
...
def _grant_currency_rewards(player_object, rewards):
    ...
    for reward in rewards:
        ...
        if key:
            player_object[key] = (player_object.get(key, 0) or 0) + (
                reward.get("amount", 0) or 0
            )
...
_INTERNAL_ALBUM_FIELDS = (
    "active_card_album",
    "last_card_album",
    "last_card_album_start_time",
    "stickers",
    "sticker_stars",
    "sticker_stars_actual",
)
...
def _defined_album_ids():
    try:
        ...
        ids = [
            _safe_int(a.get("id"))
            for a in (data.get("card_album_data") or [])
...
def open_card_packs(username, params):
    ...
    album_id = (
        params.get("card_album_id")
        or params.get("album_id")
        or _target_album_id(player_object)
    )
    ...
    return [
        ...
        (
            ...
            {"properties": _card_albums_properties(player_object)},
        ),
    ]
...
def buy_card_album_store_item(username, params):
    ...
    item = _store_items().get(
        params.get("item_id") or params.get("id") or params.get("store_item_id") or 0
    )
    if item is None:
    ...
    return [
        (
            "gs_buy_card_album_store_item",
            {
                "success": True,
                "rewards": {
                    "updateCardAlbum": True,
                    "updateCostumes": False,
                    "loot": loot,
                },
            },
        ),
...
def _collect(
    username, params, whole_album, command="gs_collect_card_album_page_rewards"
):
    root, player_object = load_player(username)
    album_id = (
        params.get("card_album_id")
        or params.get("album_id")
        or player_object.get("last_card_album", 0)
        or active_album_id()
        or 0
    )
    ...
    data = entry.setdefault("data", {})
    already = (
        bool(data.get("album_reward_collected"))
        if whole_album
        else (pages[0]["id"] in (data.get("page_rewards_collected") or []))
    )
    ...
    rewards = _grant_currency_rewards(
        player_object, [r for page in pages for r in page["rewards"]]
    )
    save_player(username, root)
...
def collect_card_album_rewards(username, params):
    return _collect(
        username, params, whole_album=True, command="gs_collect_card_album_rewards"
    )
