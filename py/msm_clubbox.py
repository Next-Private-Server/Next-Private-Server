...
def _current_celestial_entity_id():
    slot = int(time.time() // _CELESTIAL_ROTATION_WINDOW_SECONDS) % len(
        CELESTIAL_VESSEL_ENTITY_IDS
    )
...
def _resolve_island_type_uid(player_object, island_type):
    ...
    primary = [
        isl for isl in candidates if 1000 <= _safe_int(isl.get("user_island_id")) < 1100
    ]
...
def _normalize_clubbox_data(data, act_id, player_object, tokens=None):
    ...
    if act_id > 0 and _safe_int(data.get("island")) < 1:
        data["island"] = SFSLong(
            _safe_int(player_object.get("active_island")) or DEFAULT_ISLAND_UID
        )
    if act_id == _current_act_id(player_object):
        data["started_at"] = SFSLong(_current_act_event_start_date())
        data["clubbox_tokens"] = (
            _clubbox_tokens(player_object)
            if tokens is None
            else max(0, _safe_int(tokens))
        )
    elif act_id > 0:
        existing_started_at = _safe_int(data.get("started_at"))
        data["started_at"] = SFSLong(
            existing_started_at
            if existing_started_at > 0
            else _current_act_event_start_date()
        )
...
def _ensure_clubboxes(player_object):
    ...
    for act_id in range(1, current_act + 1):
        if not any(
            box.get("act") == act_id for box in clubboxes if isinstance(box, dict)
        ):
            box = _empty_clubbox()
            box["act"] = act_id
...
def _clubbox_tokens(player_object):
    return max(
        ...
        _safe_int(
            player_object.get(
                "clubbox_tokens_actual", player_object.get("clubbox_tokens", 0)
            )
        ),
    )
...
def _acts():
    if "acts" not in _cache:
        ...
        _cache["acts"] = {
            a["id"]: a for a in (data.get("clubbox_act_data") or []) if "id" in a
        }
    return _cache["acts"]
...
def _current_act_id(player_object=None):
    ...
    return max(acts.values(), key=lambda a: _safe_int(a.get("last_changed"))).get(
        "id", DEFAULT_ACT_ID
    )
    ...
    for act in data.get("clubbox_act_data") or []:
        ...
        for level in act_data.get("zoom_levels") or []:
            ...
            if "z" in level:
                level["z"] = (
                    float(level["z"])
                    if not isinstance(level["z"], dict)
                    else level["z"]
                )
...
def gs_timed_events(username, params):
    ...
    events = [
        e for e in events if str(e.get("event_type", "")) != "ClubboxAvailability"
    ]
        ...
        events.append(
            {
                ...
                "event_id": 33,
                "data": [
                    {"act": act_id, "min_client_ver": "5.5.0", "label": "DEFAULT"}
                ],
                "id": 82451,
    ...
    events = [e for e in events if str(e.get("event_type", "")) != "EvolveAvailability"]
        ...
        if str(e.get("event_type", "")) == "CardAlbum":
            ...
            for row in e.get("data") or []:
                if isinstance(row, dict):
                    ...
                    row.setdefault(
                        "label", "Sticker Book %s" % row.get("card_album_id")
                    )
...
def _clubbox_monster_reward_entry(entity_id):
    return {
        ...
        "scale": False,
        "id": entity_id,
        "type": MONSTER_TYPE_CODE,
    }


def _apply_and_format_loot(
    player_object,
    island,
    loot_dict,
    granted_card_pack,
    ...
    for key, value in loot_dict.items():
        if key in CURRENCY_LOOT_KEYS:
            ...
            if key == "clubbox_tokens":
                _set_clubbox_tokens(
                    player_object, _clubbox_tokens(player_object) + amount
                )
            else:
            ...
            entries.append(
                {
                    ...
                    "premium": False,
                    "id": pack_type,
                    "type": CARD_PACK_TYPE_CODE,
                    "extra": {"pack_type": pack_type},
                }
            )
                ...
                entries.append(
                    {
                        "amount": 1,
                        "premium": False,
                        "id": _safe_int(costume_id),
                        "type": COSTUME_TYPE_CODE,
                        "extra": {},
                    }
                )
        elif key == "monster":
            ...
            if structure_id:
                unlocked_structures = player_object.setdefault(
                    "clubbox_unlocked_structures", []
                )
...
def set_last_act(username, params):
    ...
    act_id = (
        resolved
        if resolved > 0 and resolved in _acts()
        else _current_act_id(player_object)
    )
    ...
    return {
        "success": True,
        "act_id": act_id,
        "properties": [{"clubboxes": _sanitize_clubboxes_for_wire(clubboxes)}]
...
def move_clubbox(username, params):
    ...
    if box is not None:
        ...
        save_player(username, root)
    properties = [
        {"diamonds_actual": _safe_int(player_object.get("diamonds"))},
        {"clubboxes": _sanitize_clubboxes_for_wire(clubboxes)},
    ]
    return {
        "success": True,
