...
def find_track(player_object, track_id):
    ...
    for track in _tracks(player_object):
        if (
            ...
            and int(track.get("user_track_id", 0) or 0) == track_id
        ):
            return track
    return None
...
def ensure_song(player_object, island):
    ...
    if song is None:
        song = {
            "time_denom": DEFAULT_TIME_DENOM,
            "time_numerator": DEFAULT_TIME_NUMERATOR,
            "island": island_uid,
            "tempo": DEFAULT_TEMPO,
            "key_sig": 0,
...
def track_for_monster(player_object, island, user_monster_id):
    ...
    for entry in song["tracks"]:
        if (
            ...
            and int(entry.get("monster", 0) or 0) == user_monster_id
        ):
...
def repair_composer_data(player_object):
    ...
    for island in (player_object or {}).get("islands") or []:
        ...
        if song is None:
            continue
        monster_ids = [
            int(m.get("user_monster_id", 0) or 0)
            for m in island.get("monsters") or []
            if isinstance(m, dict) and m.get("user_monster_id")
        ]
        known = {
            int(e.get("monster", 0) or 0) for e in song["tracks"] if isinstance(e, dict)
        }
...
def wire_song(song):
    ...
    return {
        "time_denom": int(
            song.get("time_denom", DEFAULT_TIME_DENOM) or DEFAULT_TIME_DENOM
        ),
        "time_numerator": int(
            song.get("time_numerator", DEFAULT_TIME_NUMERATOR) or DEFAULT_TIME_NUMERATOR
        ),
        ...
        "tracks": [
            {
                "track": int((e or {}).get("track", 0) or 0),
                "monster": int((e or {}).get("monster", 0) or 0),
            }
            for e in song.get("tracks") or []
...
def wire_songs(player_object):
    return [
        wire_song(s) for s in player_object.get("songs") or [] if isinstance(s, dict)
    ]


def wire_tracks(player_object):
    return [
        wire_track(t) for t in player_object.get("tracks") or [] if isinstance(t, dict)
    ]
...
def save_composer_track(username, params):
    ...
    if song is None:
        for island in player_object.get("islands") or []:
            if (
                is_composer_island(island)
                and int(island.get("user_island_id", 0) or 0) == island_uid
            ):
                song = ensure_song(player_object, island)
                break
    if song is not None:
    ...
    if track is None:
        ...
        if song is not None:
            ...
            if monster:
                song["tracks"].append(
                    {"track": track["user_track_id"], "monster": monster}
                )
    else:
    ...
    if track is not None:
        ...
        for song in _songs(player_object):
            song["tracks"] = [
                ...
                for e in song.get("tracks") or []
                if not (
                    isinstance(e, dict) and int(e.get("track", 0) or 0) == removed_id
                )
            ]
        save_player(username, root)
    return {"success": True}
