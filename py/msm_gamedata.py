        ...
        for data in _load_db_chunk_chain("db_costumes"):
            for entry in data.get("costume_data") or []:
                ...
                if (
                    mid is not None
                    and cid is not None
                    and mid not in _costume_by_monster_cache
                ):
...
def is_egg_holder_structure(structure_id):
    definition = get_structure_definition(structure_id)
    return bool(definition) and definition.get("structure_type") in (
        "nursery",
        "dish_harmonizer",
        "synthesizer",
    )
...
def resolve_monster_for_island(monster_id, island_type_id):
    ...
    for candidate_id, candidate in _all_monster_defs().items():
        if candidate.get("name") == name and monster_allowed_on_island(
            candidate, island_type_id
        ):
            return candidate_id
    return monster_id
...
def _paironormal_mode_entry(
    form_id, mode_value, is_active, unique_id, now, base_monster=None
):
    ...
    if is_active:
        entry["collected_starpower"] = _paironormal_starpower_owed(
            level, last_collection, now
        )
    return entry
...
def build_paironormal_modes(monster, island_type_id, island_mode, player_object=None):
    ...
    render_id = resolve_paironormal_renderable_monster_id(
        stored_id, island_type_id, island_mode
    )
    ...
    if isinstance(existing_modes, list) and existing_modes:
        ...
        has_major = any(
            isinstance(e, dict) and e.get("monster") == major_id for e in existing_modes
        )
        has_minor = any(
            isinstance(e, dict) and e.get("monster") == minor_id for e in existing_modes
        )
        placeholder_idx = next(
            (
                i
                for i, e in enumerate(existing_modes)
                if isinstance(e, dict) and not e.get("monster")
            ),
            None,
        )
            ...
            if missing_id:
                ...
                if new_uid > 0:
                    new_entry = _paironormal_mode_entry(
                        missing_id,
                        1 if missing_id == minor_id else 0,
                        True,
                        new_uid,
                        now,
                        monster,
    ...
    if player_object is not None:
        player_object["last_user_monster_id"] = max(
            int(player_object.get("last_user_monster_id", 0) or 0),
            slot0_uid,
        )
    previous_modes = monster.get("modes")
    minor_owned = bool(
        isinstance(previous_modes, list)
        and len(previous_modes) > 1
        and isinstance(previous_modes[1], dict)
        and previous_modes[1].get("a")
    )
    major_owned = bool(
        isinstance(previous_modes, list)
        and previous_modes
        and isinstance(previous_modes[0], dict)
        and previous_modes[0].get("a")
    )
    placing_minor = bool(minor_id and render_id == minor_id)
    if major_id and major_id > 0 and minor_id and minor_id > 0:
        major_entry = _paironormal_mode_entry(
            major_id, 0, major_owned or not placing_minor, slot0_uid, now, monster
        )
        minor_entry = _paironormal_mode_entry(
            minor_id, 1, minor_owned or placing_minor, slot0_uid + 1, now, monster
        )
        if player_object is not None:
            player_object["last_user_monster_id"] = max(
                int(player_object.get("last_user_monster_id", 0) or 0),
    ...
    slot0 = _paironormal_mode_entry(
        known_id, 0 if known_id == major_id else 1, True, slot0_uid, now, monster
    )
...
def _infer_monster_collection_type(existing_type, island_type, level_def):
    ...
    if island_type == AMBER_ISLAND_TYPE or (
        max_coins <= 0 and max_ethereal <= 0 and max_relics > 0
    ):
        return "relics"
...
def _breedable_rare_for_common(common_id):
    ...
    if _breedable_rares_cache is None:
        ...
        _breedable_rares_cache = {
            ...
            if entry.get("can_rarify")
            and entry.get("common_id")
            and entry.get("rare_id")
        }
...
def _breeding_result_candidates(monster_a, monster_b):
    ...
    for row in _breeding_data():
        ...
        if not (
            (a == monster_a and b == monster_b) or (a == monster_b and b == monster_a)
        ):
            continue
...
def get_max_structure_id(island_type, structure_type):
    ...
    upgrade_targets = {
        entry.get("upgrades_to") for entry in candidates if entry.get("upgrades_to")
    }
    roots = [
        entry for entry in candidates if entry["structure_id"] not in upgrade_targets
    ]
