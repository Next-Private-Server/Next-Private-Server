...
from msm_gamedata import (
    get_battle_campaign_definition,
...
def db_precheck(username, params):
    ...
    for name in list(requested) + list(_FORCE_REFETCH_TABLES) + mod_db_names():
        if (
            isinstance(name, str)
            and name.startswith("db_")
            and name not in need_updates
        ):
...
def db_minigames(username, params):
    ...
    last_updated = max(
        [_safe_int(e.get("last_changed")) for e in entries if isinstance(e, dict)]
        or [0]
    )
    return {
        "server_time": SFSLong(int(time.time() * 1000)),
    ...
    for entry in entries:
        ...
        if gene in active_set:
            ...
            patched["rare_schedule"] = {
                "timeRemaining": 0,
                "startTimes": [
                    {"duration": rotation_ms, "startTime": window_start - rotation_ms}
                ],
            }
    ...
    if isinstance(state, dict) and isinstance(state.get("total_points"), dict):
        ...
        if bits is not None:
            try:
                ...
                state["total_points"] = float(
                    struct.unpack(">d", bytes.fromhex(str(bits).removeprefix("0x")))[0]
                )
            except Exception:
...
def _repair_remapped_egg_holders(player_object):
    for island in player_object.get("islands") or []:
        ...
        broken_ids = {
            ...
            and _safe_int(structure.get("user_structure_id")) >= 2000000
        }
        if not broken_ids:
            continue
        used = {
            ...
            for structure in structures
            if isinstance(structure, dict)
            and 0 < _safe_int(structure.get("user_structure_id")) < 2000000
        }
        id_map = {}
        next_id = 1
        for structure in structures:
...
def _ensure_friend_code(player_object):
    ...
    if not nps_online.is_configured():
        ...
        code = (
            nps_online.active_friend_code() or f"{nps_online .active_account_id ():06d}"
        )
...
def _ensure_daily_calendar_structures(player_object):
    ...
    if not isinstance(player_object.get("daily_cumulative_login"), dict):
        ...
        player_object["daily_cumulative_login"] = {
            ...
            "next_collect": next_daily_reset_timestamp(),
            "reward_idx": 0,
        }
    stale_rewards = player_object.get("scaled_daily_reward")
    if not stale_rewards or len(stale_rewards) != len(_SCALED_DAILY_REWARD):
        player_object["scaled_daily_reward"] = [
            dict(entry) for entry in _SCALED_DAILY_REWARD
        ]
        player_object["_daily_bonus_fallback_idx"] = _safe_int(
            player_object.get("_daily_bonus_fallback_idx")
        ) % len(_SCALED_DAILY_REWARD)
        calendar = player_object.get("daily_cumulative_login")
        if isinstance(calendar, dict):
            calendar["reward_idx"] = _safe_int(calendar.get("reward_idx")) % len(
                _SCALED_DAILY_REWARD
            )
...
def _repair_stale_daily_calendar_id(player_object):
    ...
    if player_object.get("last_daily_bonus_claimed") and not player_object.get(
        "nextDailyLogin"
    ):
    ...
    if (
        current_type
        and current_type not in ("none", "dailyReward")
        and not player_object.get("reward_day")
    ):
...
def _backfill_daily_reward_state(player_object):
    ...
    has_real_bonus = bool(current_bonus_type) and current_bonus_type not in (
        "none",
        "dailyReward",
    )
    if not has_real_bonus:
        last_claimed = player_object.get("last_daily_bonus_claimed")
        if not last_claimed or int(time.time() * 1000) >= next_daily_reset_timestamp(
            last_claimed
        ):
...
def _clear_daily_bonus(player_object, claimed=False):
    ...
    player_object["reward_day"] = 0
    player_object["daily_reward_level"] = max(
        _safe_int(player_object.get("daily_reward_level")),
        _safe_int(player_object.get("level")),
    )
    if claimed:
...
def _find_island_by_id_or_type(islands, island_id):
    ...
    for island in islands:
        ...
        island_type = _safe_int(
            island.get("island_type") or island.get("type") or island.get("island")
        )
        resolved_island_type = msm_islands.resolve_island_type(island_type)
        if (
            user_island_id == target
            or island_type == target
            or resolved_island_type == resolved_target
        ):
            return island
...
def handle_gs_change_island(username, params):
    ...
    result = {
        "success": True,
        "user_island_id": SFSLong(resolved_user_island_id),
    }
...
def handle_gs_player(username, params):
    ...
    for _island in player_object.get("islands") or []:
        if isinstance(_island, dict):
            for _structure in _island.get("structures") or []:
                if (
                    isinstance(_structure, dict)
                    and _structure.get("structure")
                    in msm_islands.ETHEREAL_HARMONIZER_STRUCTURE_IDS
                ):
    ...
    mod_api.fire_event(
        "player_loaded", username=username, params=params, player_object=player_object
    )
    save_user_data(username, root)
    ...
    try:
        ...
        logger.warning(
            "push_snapshot/friends sync failed for %s: %s: %s",
            username,
    ...
    friends_push = {
        ...
        "requests": msm_friends.pending_wire_list(player_object),
        "tribes": [],
        "top_tribes": [],
        "global_battle_rankings": {"rankTable0": [], "rankTable1": []},
    }
    wired_player = coerce_wire_types(player_object)
    for source_island, wired_island in zip(
        player_object.get("islands") or [], wired_player.get("islands") or []
    ):
        ...
        for structure in source_island.get("structures") or []:
            if (
                ...
                and structure.get("structure")
                in msm_islands.ETHEREAL_HARMONIZER_STRUCTURE_IDS
            ):
                entry = msm_structures.dish_harmonizer_wire_data(
                    player_object, structure
                )
                if entry:
...
def _crucible_seed_frames(player_object):
    ...
    for island in player_object.get("islands") or []:
        ...
        if not wire or not (wire.get("u") or 0):
            continue
        frames.append(
            ("gs_viewed_cruc_unlock", {"success": True, "user_crucible": wire})
        )
    return frames
...
def _move_with_happiness(command, fn, structure=True):
    def handler(username, params):
        ...
        happy_effects = (
            result.pop("monster_happy_effects", None)
        ...
        if update:
            frames.append(
                ("gs_update_structure" if structure else "gs_update_monster", update)
            )
...
def _with_happy_effects(command, fn):
    def handler(username, params):
        ...
        happy_effects = (
            result.get("monster_happy_effects") if isinstance(result, dict) else None
        )
        frames = [(command, result)]
        frames += _happy_effects_frames(happy_effects)
        return frames
...
def _box_add_handler(command):
    def handler(username, params):
        ...
        if isinstance(result, dict) and result.get("success"):
            target_id = (
                result.get("user_box_monster_id")
                or params.get("user_box_monster_id")
                or result.get("user_monster_id")
            ...
            if target_id:
                try:
                    ...
                    _island, monster = find_monster_with_island(
                        player_object, target_id
                    )
                    if monster is not None:
                        if monster.get("ascend_pending"):
                            ...
                            update["powerup_unlocked"] = (
                                monster.get("powerup_unlocked") or 0
                            )
                        else:
...
def handle_player_save_profile(username, params):
    ...
    for name_key in ("display_name", "player_name", "name"):
        ...
        nested = (
            params.get("profile") if isinstance(params.get("profile"), dict) else {}
        )
...
def _set_displayname_handler(username, params):
    ...
    new_name = (
        ...
        or params.get("name")
        or params.get("user_alias")
        or ""
    )
    new_name = str(new_name or "").strip()
    if not new_name:
        return [
            ("gs_set_displayname", {"success": False, "error": "empty display name"})
        ]
    player_object["display_name"] = new_name
...
def _update_viewed_campaigns(username, params):
    ...
    if isinstance(viewed, list):
        ...
        if not isinstance(existing, list):
            existing = []
        merged = sorted(
            set(existing) | {int(v) for v in viewed if isinstance(v, (int, float))}
        )
        if merged != existing:
...
def _light_torch(username, params):
    ...
    if user_torch is None:
        next_torch_id = (
            max(
                [
                    _safe_int(torch.get("user_torch_id"))
                    for torch in torches
    ...
    player_object["total_torches_lit"] = (
        _safe_int(player_object.get("total_torches_lit")) + 1
    )
    user_id = _safe_int(
        player_object.get("bbb_id")
        or player_object.get("user_id")
        or player_object.get("user")
        or 0
    )
    save_player(username, root)
    return {
...
def _get_friends(username, params):
    ...
    try:
        ...
        logger.warning(
            "real friends list sync failed for %s: %s: %s",
            username,
            _e.__class__.__name__,
            _e,
        )
    try:
        ...
        logger.warning(
            "push_snapshot failed for %s: %s: %s", username, _e.__class__.__name__, _e
        )
    return {
        "success": True,
        "friends": friends,
...
def _send_friend_request(username, params):
    ...
    if status in (251, 388):
        entry = None
        for candidate in (
            msm_friends.real_friends_wire_list() + msm_friends.real_outgoing_wire_list()
        ):
            if (
                str(candidate.get("friend_code", "")).replace("-", "").lower()
                == code.replace("-", "").lower()
            ):
                entry = candidate
                break
        frames = notice(
            "friend_already" if status == 388 else "friend_request_sent", True
        )
        frames[0] = (
            "gs_friend_request",
            {
...
def _remove_friend(username, params):
    ...
    friend_id = _safe_int(
        _first_param(params, ("friend_id", "user_id", "userId", "bbb_id"), 0)
    )
    if not friend_id:
    ...
    if removed:
        ...
        removed = nps_online.remove_friend(
            friend_id - msm_friends.REAL_FRIEND_ID_OFFSET
        )

    frames = [
        (
            "gs_remove_friend",
            {"friend_id": SFSLong(friend_id), "success": bool(removed)},
        )
    ]
    if not removed:
        frames.append(
            ...
            if action == "accept":
                if nps_online.accept_friend_request(account_id):
                    friend = next(
                        (
                            e
                            for e in msm_friends.real_friends_wire_list()
                            if e.get("user_id") == request_id
                        ),
                        None,
                    )
                    results.append(
                ...
                gifts.append(
                    {
                        ...
                        "user_structure": SFSLong(
                            structure.get("user_structure_id", 0) or 0
                        ),
                    }
                )
    return gifts
...
_OFFICIAL_PLAYER_DEFAULTS = {
    "currency_level_scale": {
        "keys_daily_amt": 1,
        "shards_daily_amt": 9500,
        "relics_daily_amt": 3,
        "coins_daily_amt": 9350000,
        "starpower_daily_amt": 75,
        "food_daily_amt": 85000,
        "xp_daily_amt": 9350000,
    },
    "items": {"items": []},
...
def _apply_missing_official_fields(wired_player):
    ...
    if "last_minigame_tokens_converted" not in wired_player:
        wired_player["last_minigame_tokens_converted"] = SFSLong(
            int(time.time() * 1000)
        )
    return wired_player
    ...
    if isinstance(friend_object, dict):
        ...
        for island in islands:
            ...
            for slot in ("monsters", "structures", "eggs"):
                if slot in options:
                    island[slot] = (island.get(slot) or [])[
                        : max(0, int(options[slot]))
                    ]
...
def _get_friend_visit_data(username, params):
    ...
    if options.get("fail"):
        ...
        reply = _apply_visit_debug(
            _get_friend_visit_data_real(username, params), options
        )
...
def _get_friend_visit_data_real(username, params):
    ...
    root, player_object = load_player(username)
    if msm_friends.find_friend(
        player_object, target_id
    ) is None and msm_friends.is_real_friend_wire_id(target_id):
...
def _get_torchgifts(username, params):
    ...
    if not isinstance(times, list):
        times = []
    known_ids = {
        _safe_int(t.get("recipient_bbbid")) for t in times if isinstance(t, dict)
    }
    ...
    frames = [
        (
            "gs_get_torchgifts",
            {"success": False, "properties": [{"can_gift_torch_times": times}]},
        )
    ]
    ...
    if isinstance(times, list):
        ...
        for entry in times:
            if (
                ...
                and _safe_int(entry.get("recipient_bbbid")) == target_id
            ):
    ...
    if not globals().get("_DRAINING_LUA_REQUESTS", False):
        ...
        try:
            for lua_command, lua_params in mod_api.consume_lua_requests():
                logger.info(
                    "lua client request queued: %s params=%r", lua_command, lua_params
                )
                try:
...
def _handled_frames(username, command, params, frames):
    ...
    mod_api.fire_event(
        ...
        command=command,
        params=params,
        response_frames=[{"command": c, "payload": p} for c, p in frames],
    )
...
def _failure_frame(command, message, username=None):
    ...
    return [
        ...
        (
            ...
            {
                "force_logout": False,
                "msg": _generic_message(
                    "handler_error_reported" if sent else "handler_error",
                    username,
                    command=command,
                ),
            },
...
def _teleport_monster(send_home, default_destination=None):
    def handler(username, params):
        root, player_object = load_player(username)
        destination = (
            _teleport_destination_type(player_object, params)
            or default_destination
            or _BATTLE_ISLAND_TYPE
        )
        ...
        if send_home:
            result, _nursery = msm_monsters.send_monster_to_home_island(
                username, params
            )
        else:
            result, _nursery = msm_monsters.teleport_monster_to_island(
                username, forwarded
            )
        if not result.get("success"):
            return result
        frames = _egg_teleport_frames(
            username,
        ...
        if not send_home and destination in (_BATTLE_ISLAND_TYPE, _ISLAND_32_TYPE):
            ...
            happy_effects = [
                {
                    "user_monster_id": SFSLong(m["user_monster_id"]),
                    "happiness": m.get("happiness", 0) or 0,
                }
                for m in (source or {}).get("monsters") or []
                if m and m.get("user_monster_id")
            ]
            frames.append(
                (
                    "gs_multi_update_monster",
                    {"success": True, "monster_happy_effects": happy_effects},
                )
            )
        return frames
...
def _egg_teleport_frames(
    username, command, params, result, monster_id, source_island_id, monster_type
):
    ...
    frames.append(
        (
            "gs_collect_monster",
            {"coins": 0, "success": True, "user_monster_id": SFSLong(monster_id)},
        )
    )
    frames.append(
        (
    ...
    source_island = (
        _find_island(player_object.get("islands") or [], source_island_id)
        if source_island_id
        else None
    )
    if source_island is not None and monster_type:
        ...
        if sold_update is None:
            ...
            sold_update = {
                "island_id": SFSLong(source_island.get("user_island_id", 0)),
    ...
    for island in player_object.get("islands") or []:
        ...
        for candidate in island.get("monsters") or []:
            if candidate is not None and candidate.get("user_monster_id") == monster_id:
                return (
                    monster_id,
                    island.get("user_island_id", 0),
                    candidate.get("monster", 0),
                )
...
def _send_to_magical_nexus_handler(username, params):
    ...
    if not result.get("success"):
        return result
    return _egg_teleport_frames(
        username,
        "gs_send_to_magical_nexus",
        params,
        result,
        monster_id,
        source_island_id,
        ...
        frames.append(
            (
                "gs_start_synthesizing",
                {
                    ...
                    "success": True,
                    "user_synthesizing_data": started,
                    "properties": result.get("properties") or [],
                },
            )
        )
...
def _hatch_egg_handler(username, params):
    ...
    workshop_update_user_monster_id = result.pop(
        "workshop_update_user_monster_id", None
    )
    frames = [("gs_hatch_egg", result)]
    if synthesizer_collected is not None:
        frames.insert(
            0, ("gs_collect_synthesizing_success", {"structure": synthesizer_collected})
        )
    if nursery_update:
...
def _reattune_frames_handler(command, fn):
    def handler(username, params):
        result = fn(username, params)
        sold = (
            result.pop("sold_monsters_update", None)
            if isinstance(result, dict)
            else None
        )
        frames = [(command, result)]
...
def _finish_attuning_handler(username, params):
    ...
    frames = []
    tuned = (
        result.pop("tuned_monster_update", None) if isinstance(result, dict) else None
    )
    ...
    if tuned is not None:
        frames.append(
            (
                "gs_multi_update_monster",
                {"success": True, "update_monster_list": [tuned]},
            )
        )
    return frames
...
def _sell_monster_handler(username, params):
    ...
    happy_effects = (
        result.pop("monster_happy_effects", None) if isinstance(result, dict) else None
    )
    frames = []
    if sold_update:
        frames.append(("gs_update_sold_monsters", sold_update))
...
def _buy_island_handler(username, params):
    ...
    if result.get("success"):
        ...
        commands = mod_api.fire_event(
            ...
            params=params,
            player_object=player_object,
            island=result.get("user_island") or {},
        )
        if commands:
            save_player(username, root)
    return result
...
def _buy_structure_handler(username, params):
    ...
    if result.get("success"):
        ...
        commands = mod_api.fire_event(
            ...
            params=params,
            player_object=player_object,
            structure=result.get("user_structure") or {},
        )
        if commands:
            save_player(username, root)
    happy_effects = (
...
def _breed_monsters_handler(username, params):
    ...
    if result.get("success"):
        ...
        commands = mod_api.fire_event(
            ...
            params=params,
            player_object=player_object,
            breeding=result.get("user_breeding") or {},
        )
        if commands:
            save_player(username, root)
    return result
...
def _start_fuzing_handler(username, params):
    ...
    if result.get("success"):
        ...
        commands = mod_api.fire_event(
            ...
            params=params,
            player_object=player_object,
            fuzing=result.get("user_fuzing") or {},
        )
        if commands:
            save_player(username, root)
    return result
...
def _collect_daily_reward(username, params):
    root, player_object = load_player(username)
    logger.info(
        "DAILY_REWARD_DEBUG collect: bonus_type=%r amount=%r last_claimed=%r instant_timers=%r",
        player_object.get("daily_bonus_type"),
        player_object.get("daily_bonus_amount"),
        player_object.get("last_daily_bonus_claimed"),
        msm_toggles.is_enabled("instant_timers"),
    )
    bonus_type = player_object.get("daily_bonus_type") or "none"
    bonus_amount = _safe_int(player_object.get("daily_bonus_amount"))
    granted = (
        bonus_type
        and bonus_type not in ("none", "dailyReward", "deferred")
        and bonus_amount > 0
    )
    if not granted:
        _backfill_daily_reward_state(player_object)
    ...
    logger.info(
        "DAILY_REWARD_DEBUG granted=%r type=%r amount=%r day=%r",
        granted,
        bonus_type,
        bonus_amount,
        player_object.get("reward_day"),
    )
    if granted:
        currency_map = {
            "coin": "coins",
            ...
            "diamonds": "diamonds",
            "food": "food",
            "shard": "ethereal_currency",
            "shards": "ethereal_currency",
            "ethereal": "ethereal_currency",
            "key": "keys",
            "keys": "keys",
            "relic": "relics",
            "relics": "relics",
...
def _collect_daily_cumulative_login_rewards(username, params):
    ...
    if bonus_type != "none" and bonus_amount > 0:
        player_object[bonus_type] = (
            _safe_int(player_object.get(bonus_type)) + bonus_amount
        )
        actual_key = f"{bonus_type }_actual"
        if actual_key in player_object:
            player_object[actual_key] = (
                _safe_int(player_object.get(actual_key)) + bonus_amount
            )
    calendar["reward_idx"] = _safe_int(calendar.get("reward_idx")) + 1
    calendar["total"] = _safe_int(calendar.get("total")) + 1
    calendar["next_collect"] = next_daily_reset_timestamp()
    ...
    type_code = _DAILY_CALENDAR_CURRENCY_TYPE_CODES.get(
        bonus_type, _DAILY_CALENDAR_GENERIC_CURRENCY_TYPE_CODE
    )
...
def _welcome_mail(now, username):
    ...
    return {
        ...
        "author": sender_name,
        "icon": "mail",
        "received": SFSLong(now),
        "received_on": SFSLong(now),
        "expiry": 0,
        "urgent": False,
        "read": False,
    }
...
def _ensure_mailbox(player_object, username=None):
    ...
    if not isinstance(mailbox, list):
        mailbox = []
    if not any(
        (m or {}).get("message_id") == 1 and (m or {}).get("user_mail_id") == 1
        for m in mailbox
...
def _delete_mail(username, params):
    ...
    entry = next(
        (
            m
            for m in mailbox
            if isinstance(m, dict) and _safe_int(m.get("id")) == mail_id
        ),
        None,
    )
    properties = []
    if entry is not None:
        ...
        player_object["mailbox"] = mailbox
        unread = sum(
            1 for m in mailbox if isinstance(m, dict) and not m.get("read", False)
        )
        player_object["num_unread_mail"] = unread
        player_object["unread_mail_count"] = unread
        player_object["new_mail"] = unread > 0
...
GAMEPLAY_HANDLERS = {
    ...
    "gs_move_structure": _move_with_happiness(
        "gs_move_structure", msm_structures.move_structure, structure=True
    ),
    "move_structure": _with_structure_update(
        "move_structure", msm_structures.move_structure, always=True
    ),
    "gs_update_structure_position": _with_structure_update(
        "gs_update_structure_position", msm_structures.move_structure, always=True
    ),
    "gs_sell_structure": _with_happy_effects(
        "gs_sell_structure", msm_structures.sell_structure
    ),
    "gs_remove_obstacle": _simple(msm_structures.clear_obstacle),
    "gs_clear_obstacle": _simple(msm_structures.clear_obstacle),
    "gs_start_obstacle": _with_structure_update(
        "gs_start_obstacle", msm_structures.start_obstacle
    ),
    "gs_collect_nucleus_reward": _simple(msm_structures.collect_nucleus_reward),
    "gs_clear_obstacle_speed_up": _with_structure_update(
        "gs_clear_obstacle_speed_up", msm_structures.clear_obstacle_speed_up
    ),
    "gs_speed_up_clear_obstacle": _with_structure_update(
        "gs_speed_up_clear_obstacle", msm_structures.clear_obstacle_speed_up
    ),
    "gs_speedup_clear_obstacle": _with_structure_update(
        "gs_speedup_clear_obstacle", msm_structures.clear_obstacle_speed_up
    ),
    "gs_speed_up_obstacle": _with_structure_update(
        "gs_speed_up_obstacle", msm_structures.clear_obstacle_speed_up
    ),
    "gs_skip_obstacle": _with_structure_update(
        "gs_skip_obstacle", msm_structures.clear_obstacle_speed_up
    ),
    "gs_finish_structure": _with_structure_update(
        "gs_finish_structure", msm_structures.finish_structure
    ),
    ...
    "gs_flip_structure": _with_structure_update(
        "gs_flip_structure", msm_structures.flip_structure, always=True
    ),
    "gs_mute_structure": _structure_update_only(msm_structures.mute_structure),
    "gs_start_upgrade_structure": _structure_update_only(
        _second_only(msm_structures.start_upgrade_structure)
    ),
    "gs_upgrade_structure": _structure_update_only(
        _second_only(msm_structures.start_upgrade_structure)
    ),
    "gs_update_structure": _simple(msm_structures.update_structure),
    "gs_speed_up_structure": _with_structure_update_first(
        "gs_speed_up_structure", msm_structures.speed_up_structure
    ),
    "gs_finish_upgrade_structure": _with_structure_update(
        "gs_finish_upgrade_structure", msm_structures.finish_upgrade_structure
    ),
    "gs_speed_up_upgrade_structure": _with_structure_update_first(
        "gs_speed_up_upgrade_structure", msm_structures.speed_up_upgrade_structure
    ),
    "gs_speedup_upgrade_structure": _with_structure_update_first(
        "gs_speedup_upgrade_structure", msm_structures.speed_up_upgrade_structure
    ),
    "gs_speed_up_upgrade_structure_video": _with_structure_update_first(
        "gs_speed_up_upgrade_structure_video", msm_structures.speed_up_upgrade_structure
    ),
    "gs_speedup_upgrade_structure_video": _with_structure_update_first(
        "gs_speedup_upgrade_structure_video", msm_structures.speed_up_upgrade_structure
    ),
    "gs_start_fuguing": _with_structure_update(
        "gs_start_fuguing", msm_structures.start_fuguing
    ),
    "gs_finish_fuguing": _finish_fuguing_handler,
    "gs_merge_fugue": _with_structure_update(
        "gs_merge_fugue", msm_structures.merge_fugue
    ),
    "gs_speed_up_fuguing": _with_structure_update(
        "gs_speed_up_fuguing", msm_structures.speed_up_fuguing
    ),
    "gs_speedup_fuguing": _with_structure_update(
        "gs_speedup_fuguing", msm_structures.speed_up_fuguing
    ),
    "gs_viewed_fugued_monster": _simple(msm_structures.viewed_fugued_monster),
    "gs_collect_structure": _with_structure_update_event(
        "structure_collected", "gs_collect_structure", msm_structures.collect_structure
    ),
    "gs_collect_from_mine": _with_structure_update_event(
        "mine_collected", "gs_collect_from_mine", msm_structures.collect_mine
    ),
    ...
    "gs_move_monster": _move_with_happiness(
        "gs_move_monster", msm_monsters.move_monster, structure=False
    ),
    "move_monster": _with_monster_update("move_monster", msm_monsters.move_monster),
    "gs_update_monster_position": _with_monster_update(
        "gs_update_monster_position", msm_monsters.move_monster
    ),
    "gs_flip_monster": _with_monster_update(
        "gs_flip_monster", msm_monsters.flip_monster
    ),
    "gs_mute_monster": _with_monster_update(
        "gs_mute_monster", msm_monsters.mute_monster
    ),
    "gs_add_soul_link": _with_monster_update_first(
        "gs_add_soul_link", msm_monsters.add_soul_link
    ),
    "gs_remove_soul_link": _with_monster_update_first(
        "gs_remove_soul_link", msm_monsters.remove_soul_link
    ),
    "gs_toggle_titansoul_fx": _with_monster_update_first(
        "gs_toggle_titansoul_fx", msm_monsters.toggle_titansoul_fx
    ),
    ...
    "gs_feed_monster": _with_monster_update_event(
        "monster_fed", "gs_feed_monster", msm_monsters.feed_monster
    ),
    ...
    "gs_collect_multi_monster": _collect_monster(
        "gs_collect_multi_monster", msm_monsters.collect_multi_monster
    ),
    ...
    "gs_speed_up_breeding": _with_structure_update(
        "gs_speed_up_breeding", msm_monsters.speed_up_breeding
    ),
    "gs_speed_up_breeding_video": _with_structure_update(
        "gs_speed_up_breeding_video", msm_monsters.speed_up_breeding
    ),
    "gs_speedup_breeding": _with_structure_update(
        "gs_speedup_breeding", msm_monsters.speed_up_breeding
    ),
    "gs_speedup_breeding_video": _with_structure_update(
        "gs_speedup_breeding_video", msm_monsters.speed_up_breeding
    ),
    "gs_speedup_breed_monsters": _with_structure_update(
        "gs_speedup_breed_monsters", msm_monsters.speed_up_breeding
    ),
    "gs_speedup_breeding_monsters": _with_structure_update(
        "gs_speedup_breeding_monsters", msm_monsters.speed_up_breeding
    ),
    ...
    "gs_speed_up_fuzing": _with_structure_update(
        "gs_speed_up_fuzing", msm_monsters.speed_up_fuzing
    ),
    "gs_finish_fuzing": _finish_fuzing_handler,
    "gs_store_buddy": _simple(msm_monsters.store_buddy),
    "gs_unstore_buddy": _with_structure_update(
        "gs_unstore_buddy", msm_monsters.unstore_buddy
    ),
    ...
    "gs_finish_amber_evolve": _with_monster_update(
        "gs_finish_amber_evolve",
        lambda u, p: msm_monsters.finish_amber_evolve(u, p, True),
    ),
    ...
    "gs_collect_cruc_heat": _with_monster_update(
        "gs_collect_cruc_heat", msm_islands.collect_crucible_heat
    ),
    ...
    "battle_teleport": _teleport_monster(
        False, default_destination=_BATTLE_ISLAND_TYPE
    ),
    ...
    "gs_finish_dish_harmonizing": _with_structure_update(
        "gs_finish_dish_harmonizing",
        msm_structures.finish_dish_harmonizing,
        always=True,
    ),
    ...
    "battle_start_training": _with_monster_update(
        "battle_start_training", msm_monsters.battle_start_training
    ),
    "battle_finish_training": _with_monster_update(
        "battle_finish_training", msm_monsters.battle_finish_training
    ),
    ...
    "gs_update_dish_harmonizer_target": _with_structure_update(
        "gs_update_dish_harmonizer_target",
        msm_structures.update_dish_harmonizer_target,
        always=True,
    ),
    "gs_incubate_dish_harmonizer_egg": _with_structure_update(
        "gs_incubate_dish_harmonizer_egg",
        msm_structures.incubate_dish_harmonizer_egg,
        always=True,
    ),
    ...
    "gs_set_islandname": _set_island_field_handler(
        "name", ("name", "island_name", "islandName")
    ),
    "gs_set_moniker": _set_player_field_handler("moniker", ("moniker", "name")),
    "gs_set_avatar": _set_player_field_handler(
        "avatar", ("avatar", "avatar_id", "avatarId")
    ),
    "gs_set_tribename": _set_player_field_handler(
        "tribe_name", ("name", "tribe_name", "tribeName")
    ),
    "gs_set_last_card_album": _set_player_field_handler(
        "last_card_album", ("card_album_id", "album_id", "last_card_album")
    ),
    "gs_set_light_torch_flag": _set_island_field_handler(
        "light_torch_flag", ("light_torch_flag", "flag", "value")
    ),
    ...
    "gs_memory_minigame_current_cost": _simple(
        msm_rewards.memory_minigame_current_cost
    ),
    ...
    "gs_box_purchase_fill": _with_monster_update(
        "gs_box_purchase_fill", msm_box.box_purchase_fill
    ),
    "gs_box_activate_monster": _box_activate_handler(
        "gs_box_activate_monster", msm_box.box_activate_monster
    ),
    "gs_activate_box_monster": _box_activate_handler(
        "gs_activate_box_monster", msm_box.wake_wubbox
    ),
    "gs_wake_wubbox": _box_activate_handler("gs_wake_wubbox", msm_box.wake_wubbox),
    "gs_attempt_early_box_activate": _box_activate_handler(
        "gs_attempt_early_box_activate", msm_box.attempt_early_box_activate
    ),
    "gs_purchase_evolve_unlock": _box_activate_handler(
        "gs_purchase_evolve_unlock", msm_box.purchase_evolve_unlock
    ),
    "gs_purchase_evo_powerup_unlock": _box_activate_handler(
        "gs_purchase_evo_powerup_unlock", msm_box.purchase_evo_powerup_unlock
    ),
    ...
    "gs_speedup_dish_harmonizing": _with_structure_update(
        "gs_speedup_dish_harmonizing",
        msm_structures.speed_up_dish_harmonizing,
        always=True,
    ),
    "gs_start_dish_harmonizing": _with_structure_update(
        "gs_start_dish_harmonizing", msm_structures.start_dish_harmonizing, always=True
    ),
    ...
    "gs_collect_reattune_monster": _reattune_frames_handler(
        "gs_collect_reattune_monster", msm_attune.collect_reattune_monster
    ),
    "gs_viewed_reattuned_monster": _reattune_frames_handler(
        "gs_viewed_reattuned_monster", msm_attune.viewed_reattuned_monster
    ),
    ...
    "gs_collect_synthesizing_success": _simple(
        msm_synthesis.collect_synthesizing_success
    ),
    "gs_collect_synthesizing_failure": _simple(
        msm_synthesis.collect_synthesizing_failure
    ),
    ...
    "update_awakener": _with_structure_update(
        "update_awakener", msm_structures.update_awakener
    ),
    "gs_collect_card_album_rewards": _simple(msm_cardalbum.collect_card_album_rewards),
    "gs_collect_card_album_page_rewards": _simple(
        msm_cardalbum.collect_card_album_page_rewards
    ),
    "card_album_reward_collect": _simple(msm_cardalbum.collect_card_album_rewards),
    "card_album_page_reward_collect": _simple(
        msm_cardalbum.collect_card_album_page_rewards
    ),
...
def handle_login(params):
    ...
    return _append_mod_frames(
        resolved, [("USER_LOGIN", {"data": {}, "success": True, "user": resolved})]
    )
...
def login_bootstrap_frames():
    ...
    if gs_initialized is not None:
        frames.append(
            (
                "gs_initialized",
                normalize_db_payload("gs_initialized", dict(gs_initialized)),
            )
        )
    frames.append(
        (
            ...
            {
                "force_logout": False,
                "msg": _generic_message("welcome", get_active_username()),
            },
        )
    )
    return frames
...
def handle_command(command, params):
    ...
    try:
        mod_api.fire_event(
            "command_received", username=active_username, command=command, params=params
        )
    except Exception:
    ...
    if aliased_command is not None:
        ...
        if data is None:
            logger.info(
                "no captured response for %s (via %s)", command, aliased_command
            )
    ...
    if handler is not None:
        ...
        try:
            ...
            return _handled_frames(
                active_username,
                command,
                params,
                _failure_frame(command, exc, active_username),
            )
        if result is None:
    ...
    if not command.startswith("db_") and mod_api.has_action(command):
        logger.info(
            "command %s handled by registered java mod action before db capture",
            command,
        )
        mod_frames = mod_api.dispatch_action(command, active_username, params)
        if mod_frames:
            logger.info(
                "command %s java mod action produced %d frame(s)",
                command,
                len(mod_frames),
            )
    ...
    if data is None:
        ...
        if mod_frames:
            logger.info(
                "command %s handled by java mod action with %d frame(s)",
                command,
                len(mod_frames),
            )
            return _handled_frames(active_username, command, params, mod_frames)
        ...
        if "card" in command.lower():
            ...
            return _handled_frames(
                active_username, command, params, [(command, {"success": True})]
            )

        notice = {
            "force_logout": False,
            "msg": _generic_message(
                "unhandled_command", active_username, command=command
            ),
        }
        return _handled_frames(
            active_username, command, params, [("gs_display_generic_message", notice)]
        )
