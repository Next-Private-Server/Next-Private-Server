...
get_island_definition ,get_max_structure_id ,get_monster_definition ,get_monster_id_for_entity_id ,
get_structure_definition ,get_user_game_setting_int ,island_for_theme ,is_egg_holder_structure ,
load_db_json ,resolve_island_type ,
)
...
def _make_crucible_structure (user_island_id ,structure_id ,pos_x ,pos_y ,now ):
    return {
    "island":user_island_id ,"date_created":now ,
    "scale":1.0 ,"last_collection":now ,
    "structure":structure_id ,"pos_y":pos_y ,
    "is_upgrading":0 ,"user_structure_id":10000 +random .randint (0 ,989999 ),
    "pos_x":pos_x ,"in_warehouse":0 ,"is_complete":1 ,
    "building_completed":now ,"muted":0 ,"flip":0 ,
    }
def _is_crucible_structure (structure ):
    definition =get_structure_definition (structure .get ("structure",structure .get ("structure_id",0 )))or {}
...
def repair_crucible_structure (island ):
    ...
    structures .append (_make_crucible_structure (
    island .get ("user_island_id",0 )or 0 ,CRUCIBLE_STRUCTURE_ID ,CRUCIBLE_POSITION [0 ],CRUCIBLE_POSITION [1 ],now ,
    ))
...
def _crucible_wire_object (island ,structure ,overrides =None ):
    record =_find_amber_evolve_record_on_island (island ,structure .get ("user_structure_id",0 ))
...
def collect_crucible_heat (username ,params ):
    ...
    result ={
    "success":True ,"user_crucible":_crucible_wire_object (island ,structure ),
    "collected_relics":relics_spent ,"user_monster_id":SFSLong (user_monster_id ),
...
def default_island_structures (user_island_id ,island_type ):
    ...
    if island_type in ETHEREAL_ISLAND_TYPES :
        ...
        entries =[(ETHEREAL_DISH_HARMONIZER_STRUCTURE_ID ,harmonizer_pos [0 ],harmonizer_pos [1 ],0 )]
        if island_type in ETHEREAL_ISLAND_TYPES_WITH_EGGCUPS :
            entries .extend ((ETHEREAL_EGGCUP_STRUCTURE_ID ,x ,y ,0 )for x ,y in ETHEREAL_EGGCUP_POSITIONS_BY_TYPE [island_type ])
        return [
        _make_structure (user_island_id ,island_type ,sid ,x ,y ,flip ,
        (get_structure_definition (sid )or {}).get ("level",1 )or 1 ,now )
    ...
    if island_type ==AMBER_ISLAND_TYPE :
        structures .append (_make_crucible_structure (
        user_island_id ,CRUCIBLE_STRUCTURE_ID ,CRUCIBLE_POSITION [0 ],CRUCIBLE_POSITION [1 ],now ,
        ))
    return structures
...
def backfill_nps_test_structure(island):
    ...
    for i in range(len(structures) - 1, -1, -1):
        if structures[i] is not None and structures[i].get("structure") == NPS_TEST_STRUCTURE_ID:
            del structures[i]
...
def repair_mirror_island_type (player_object ):
    ...
    for island in player_object .get ("islands")or []:
        ...
        if island_field is None :
            continue
        if island .get ("type")!=island_field or island .get ("island_type")!=island_field :
...
def repair_ethereal_structure_positions (island ):
    island_type =island .get ("island_type",island .get ("type",island .get ("island",0 )))or 0
    if island_type not in ETHEREAL_ISLAND_TYPES :
        return
    if int (island .get ("ethereal_positions_repair_version",0 )or 0 )>=ETHEREAL_POSITION_REPAIR_VERSION :
        return
...
_STALE_PLAYER_GENE_KEYS =("attuner_genes","attunerGenes","attuner_gene_inventory","attunerGenesData","meebs")
def repair_ethereal_workshop_state (player_object ,island ):
    ...
    for entry in critters :
        ...
        if not gene :
            continue
        best =max (entry .get ("num",0 )or 0 ,entry .get ("count",0 )or 0 ,entry .get ("amount",0 )or 0 )
...
def repair_dish_harmonizer_state (island ):
    ...
    if island_type not in ETHEREAL_ISLAND_TYPES :
        return
    for structure in island .get ("structures")or []:
        if not isinstance (structure ,dict )or structure .get ("structure")not in ETHEREAL_HARMONIZER_STRUCTURE_IDS :
            continue
        ...
        if targets and not structure .get ("dish_harmonizer_complete_on"):
            structure ["dish_harmonizer_complete_on"]=structure .get ("building_completed",0 )or 0
...
def backfill_ethereal_eggcups (island ):
    ...
    if any (s is not None and s .get ("structure")==804 for s in structures ):
        island ["structures"]=[s for s in structures if s is None or s .get ("structure")!=804 ]
        structures =island ["structures"]
    if int (island .get ("eggcup_repair_version",0 )or 0 )>=ETHEREAL_EGGCUP_REPAIR_VERSION :
        return
    ...
    eggcup_count =sum (1 for s in structures if s is not None and s .get ("structure")==ETHEREAL_EGGCUP_STRUCTURE_ID )
    if eggcup_count <len (eggcup_positions ):
        ...
        for x ,y in eggcup_positions :
            ...
            if (x ,y )in occupied :
                continue
            structures .append (_make_structure (user_island_id ,island_type ,ETHEREAL_EGGCUP_STRUCTURE_ID ,x ,y ,0 ,level ,now ))
            occupied .add ((x ,y ))
            eggcup_count +=1
...
"monsters","structures","eggs","breeding","baking","torches","fuzer",
"fuzer_input_buddies","amber_evolve","costumes_owned","tiles","book_monster_ids","boxed_eggs",
)
...
def repair_preset_mirror_island_collisions (player_object ):
    ...
    for island in islands :
        ...
        if uid <=0 or uid >=100000000 :
            continue
        island_type =island .get ("island_type",island .get ("type",island .get ("island")))
...
def repair_duplicate_island_types (player_object ):
    ...
    for uid ,group in groups .items ():
        ...
        for other in group :
            ...
            if other_monsters :
                primary ["num_monsters"]=(primary .get ("num_monsters",0 )or 0 )+other_monsters
...
def repair_broken_clubbox_eggs (player_object ):
    for island in player_object .get ("islands")or []:
        ...
        if len (kept )!=len (eggs ):
            island ["eggs"]=kept
            for structure in island .get ("structures")or []:
                if structure is not None and structure .get ("user_structure_id")in freed_structure_ids :
...
def repair_structure_build_timestamps (island ):
    ...
    for structure in island .get ("structures")or []:
        ...
        if isinstance (structure_def ,int )and structure_def >0 and structure .get ("structure_id")!=structure_def :
        ...
        if definition .get ("structure_type")=="obstacle":
            if (structure .get ("building_completed")or 0 )<= (structure .get ("date_created")or 0 ):
                for key in ("date_created","building_completed","obj_data","obj_end","finishing_time"):
        ...
        if not structure .get ("building_completed"):
            structure ["building_completed"]=SFSLong (structure .get ("date_created")or now )
...
def repair_structure_definition_aliases (player_object ):
    ...
    for island in player_object .get ("islands")or []:
        ...
        for structure in island .get ("structures")or []:
            ...
            if isinstance (structure_def ,int )and structure_def >0 and alias !=structure_def :
                structure ["structure_id"]=structure_def
                changed =True
    if changed :
...
def repair_low_id_structures (player_object ):
    if player_object .get ("low_id_structures_repaired"):
        return
    islands =player_object .get ("islands")or []
    existing_ids ={s .get ("user_structure_id")for isl in islands if isl is not None
    for s in (isl .get ("structures")or [])if s is not None }
    for island in islands :
        if island is None :
            continue
...
def purge_direct_placement_eggs (player_object ):
    ...
    for island in player_object .get ("islands")or []:
        ...
        kept =[]
        for egg in eggs :
            phantom =_is_direct_placement_egg (egg )or (purge_legacy and _is_legacy_direct_placement_egg (egg ))
            if phantom and not requires_direct_placement (get_monster_definition ((egg or {}).get ("monster",0 )),island_type ):
                continue
            kept .append (egg )
        if len (kept )!=len (eggs ):
            island ["eggs"]=kept
    ...
    for island in player_object .get ("islands")or []:
        ...
        def first_egg_holder (used_ids =()):
            ...
            for structure in structures :
                ...
                if sid in used_ids :
                    continue
                if is_egg_holder_structure (structure .get ("structure",structure .get ("structure_id",0 ))or 0 )and structure_type (structure )not in ("synthesizer","dish_harmonizer"):
                    return structure
            return None
        ...
        for egg in eggs :
            ...
            if holder is None or not is_egg_holder_structure (holder .get ("structure",holder .get ("structure_id",0 ))or 0 ):
            ...
            if structure_type (holder )!="synthesizer":
                ...
                holder ["obj_data"]=1
                holder ["obj_end"]=egg .get ("hatches_on",holder .get ("obj_end",now )or now )
                holder ["finishing_time"]=egg .get ("hatches_on",holder .get ("finishing_time",holder ["obj_end"])or holder ["obj_end"])
        ...
        for structure in structures :
            ...
            if structure_type (structure )=="synthesizer":
                for key in ("occupied","has_egg","viewed","obj_data","obj_end","finishing_time"):
                    if structure .pop (key ,None )is not None :
                        changed =True
                continue
            if structure_type (structure )=="dish_harmonizer":
                continue
            if is_egg_holder_structure (structure .get ("structure",structure .get ("structure_id",0 ))or 0 )and sid not in valid_egg_holder_ids :
        ...
        breeding =[record for record in island .get ("breeding")or []if record is not None ]
        repaired_breeding =[]
...
def buy_island (username ,params ):
    ...
    "num_torches":0 ,"user_island_id":SFSLong (user_island_id ),"user":SFSLong (existing_user ),
    "type":island_id ,"island_type":island_id ,
    }
    if island_type ==9 :
        ...
        "user_island_id":SFSLong (user_island_id ),"chief":SFSLong (existing_user ),
...
def buy_island_skin (username ,params ):
    ...
    theme_id =None
    for key in ("island_theme_id","user_island_theme_id","theme_id","skin_id","skin","theme","id","storeitem_id"):
    ...
    "island":island_type ,"user_island_theme_id":reported_theme_id ,"theme_id":reported_theme_id ,"skin_id":reported_theme_id ,
