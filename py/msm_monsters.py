...
import msm_toggles
from msm_box import (
UNDERLING_ISLAND_TYPES ,BOX_SIMPLE_ISLAND_TYPES ,apply_awakened_box_state ,apply_inactive_box_state ,apply_underling_box_state ,
build_placeholder_box_monster ,is_box_monster_entity ,requires_direct_placement ,
)

def _base_build_ms (monster_id ):
    return int ((get_monster_definition (monster_id )or {}).get ("build_time",0 )or 0 )*1000
...
def _sell_refund (book_value ):
    ...
    if book_value <=0 :
        return 0
    percentage =msm_toggles .get_int ("sell_percentage",75 ,minimum =0 ,maximum =100 )/100.0
...
all_monster_ids ,build_paironormal_modes ,choose_breeding_result_monster ,compute_monster_economy ,
find_paironormal_form_id ,get_costume_definition ,get_costume_id_for_monster ,get_max_monster_level ,get_monster_definition ,
get_monster_id_for_entity_id ,
get_user_game_setting_int ,soul_link_coin_costs ,
get_monster_level_definition ,get_structure_definition ,is_egg_holder_structure ,is_nursery_structure ,
is_paironormal_island ,is_paironormal_multimodal ,monster_allowed_on_island ,
...
SFSLong ,action_result ,add_actual_currencies ,create_player_properties ,diamond_speedup_cost ,
...
append_inventory_property ,consume_inventory_item ,
)
...
def _safe_float (value ,default =0.0 ):
    if isinstance (value ,dict ):
        ...
        if hex_bits is not None :
            try :
                return float (struct .unpack (">f",bytes .fromhex (str (hex_bits ).removeprefix ("0x"))[-4 :])[0 ])
            except (TypeError ,ValueError ,struct .error ):
                return default
    try :
        return float (value )
...
def _structure_type (structure ):
    if structure is None :
        return ""
    definition =get_structure_definition (structure .get ("structure",structure .get ("structure_id",0 ))or 0 )or {}
...
def repair_magical_nexus_layout (island ):
    if island is None or island_type_of (island )!=MAGICAL_NEXUS_ISLAND_TYPE :
        return False
    changed =False
    island_uid =_safe_int (island .get ("user_island_id")or island .get ("island"),1000 +MAGICAL_NEXUS_ISLAND_TYPE )
    remapped ={}
    now =int (time .time ()*1000 )
    for structure in island .get ("structures")or []:
        if structure is None :
            continue
        structure_type =_safe_int (structure .get ("structure")or structure .get ("structure_id"))
        ...
        if structure .get ("structure")!=structure_type :
            ...
            changed =True
        for key ,value in (("island",island_uid ),("user_island_id",island_uid ),("in_warehouse",0 ),("is_upgrading",0 ),("is_complete",1 ),("muted",0 ),("flip",0 )):
        ...
        if _safe_int (structure .get ("building_completed"))<=0 :
            structure ["building_completed"]=_safe_int (structure .get ("date_created"),now )
            changed =True
        if is_nursery_structure (structure_type ):
            held_egg =next ((e for e in island .get ("eggs")or []if e is not None and _safe_int (e .get ("structure"))==desired_id ),None )
            hatches_on =_safe_int (held_egg .get ("hatches_on"))if held_egg else 0
            for key ,value in (("occupied",held_egg is not None ),("has_egg",held_egg is not None ),
            ("obj_data",1 if held_egg is not None else 0 ),("obj_end",hatches_on )):
                if structure .get (key )!=value :
                    structure [key ]=value
                    changed =True
    ...
    for island in player_object .get ("islands")or []:
        ...
        for monster in island .get ("monsters")or []:
            ...
            if is_box_monster_entity (definition ):
                continue
            entry ={"monsterId":monster .get ("monster"),"level":monster .get ("level",1 )or 1 }
...
def backfill_battle_state (player_object ):
    ...
    if not isinstance (player_object .get ("battle"),dict ):
        ...
        "level":1 ,"max_training_level":15 ,"xp":0 ,
        "loadout":"{}","loadout_versus":loadout_json ,
        }
    if not player_object .get ("battle_versus"):
        now =int (time .time ()*1000 )
        player_object ["battle_versus"]=[{
        "campaign_id":BATTLE_VERSUS_CAMPAIGN_ID ,"tier":1 ,
        "attempts":BATTLE_VERSUS_MAX_ATTEMPTS ,
        "stars":0 ,"streak":0 ,"match_score":1500 ,"champion_score":0 ,
        "user_id":player_object .get ("bbb_id",0 )or 0 ,
        "display_name":player_object .get ("display_name","")or "",
        "started_on":now ,"schedule_started_on":now ,
        "refreshes_on":now +86400000 ,
        "loadout":loadout_json ,"last_opponent_loadout":loadout_json ,"last_seed":0 ,
        }]
    else :
        now =int (time .time ()*1000 )
...
def _paironormal_sold_ids (island ,monster_type ):
    ...
    if island_type_of (island )!=PAIRONORMAL_ISLAND_TYPE or not monster_type :
        return ids
    from msm_gamedata import find_paironormal_form_id ,resolve_paironormal_stored_monster_id
    parent =resolve_paironormal_stored_monster_id (monster_type ,PAIRONORMAL_ISLAND_TYPE )
...
def _build_buyback_data (monster ):
    ...
    "coin_cost":_safe_int (monster .get ("book_value"))or _safe_int (definition .get ("cost_coins")),
    }
def purchase_buyback (username ,params ):
    ...
    "island":SFSLong (island_id ),"monster":monster_type ,
    "pos_x":x_pos ,"pos_y":y_pos ,"flip":flip ,"muted":0 ,
    "level":slot .get ("level",1 ),"happiness":0 ,"times_fed":slot .get ("times_fed",0 ),
    "name":slot .get ("name","")or (get_monster_definition (monster_type )or {}).get ("common_name",""),
    "in_hotel":0 ,"volume":SFSFloat (1.0 ),
...
def _monster_update (monster ,mode ="full"):
    ...
    if mode =="move":
        ...
        update ["collected_relics"]=SFSFloat (_safe_float (monster .get ("collected_relics"),0.0 ))
        ...
        update ["training_complete_on"]=SFSLong (monster .get ("training_complete_on",0 )or 0 )
    return update
...
def biggify_monster (username ,params ):
    ...
    if enabled and not was_mega and msm_toggles .is_enabled ("functioning_currencies"):
        ...
        "USER_DIAMOND_COST_PER_PERMALIT_MEGAMONSTER"if permamega else "USER_DIAMOND_COST_PER_DAILY_MEGAMONSTER",
...
def sell_monster (username ,params ):
    ...
    if island is not None :
        ...
        if monster is not None and msm_toggles .is_enabled ("functioning_currencies"):
            book_value =monster .get ("book_value")or (get_monster_definition (monster_type )or {}).get ("cost_coins",0 )or 0
...
def _charge_soul_link_cost (player_object ,link_index ):
    ...
    if link_index <len (costs ):
        ...
        increment =get_user_game_setting_int ("USER_SOUL_LINK_DIAMOND_INCREMENT_COST",0 )
...
def _battle_training_levels (monster_id ):
    if "data"not in _battle_training_cache :
        ...
        row .get ("monster_id"):sorted (row .get ("levels")or [],key =lambda lv :lv .get ("level",0 ))
...
def battle_start_training (username ,params ):
    ...
    next_def =next ((lv for lv in levels if lv .get ("level",0 )==current_level +1 ),None )
    if next_def is None :
    ...
    if (player_object .get ("coins",0 )or 0 )<cost :
        ...
        return result ,{}
    now =int (time .time ()*1000 )
    training_ms =0 if msm_toggles .is_enabled ("instant_timers")else max (0 ,_safe_int (next_def .get ("training_time")))*1000
...
def battle_finish_training (username ,params ):
    ...
    new_level =monster .get ("pending_battle_level",max (1 ,monster .get ("level",1 )or 1 )+1 )
    ...
    return result ,_monster_update (monster ,"training")
_COLLECTION_PLAYER_KEY ={"ethereal_currency":"ethereal_currency","starpower":"starpower","egg_wildcards":"egg_wildcards","relics":"relics"}
_COLLECTION_STORED_FIELD ={"ethereal_currency":"collected_ethereal","relics":"collected_relics"}
...
def _collection_stored_field (collection_type ):
    return _COLLECTION_STORED_FIELD .get (_normalize_collection_type (collection_type ),"collected_coins")
    ...
    for island in player_object .get ("islands")or []:
        ...
        for monster in island .get ("monsters")or []:
            ...
            for mode_index ,mode in enumerate (monster .get ("modes")or []):
                if isinstance (mode ,dict )and mode .get ("user_monster_id")==mode_unique_id :
...
def _collect_paironormal_monster (username ,params ,monster ):
    now =int (time .time ()*1000 )
    root ,player_object =load_player (username )
    _island ,fresh_monster =find_monster_with_island (player_object ,monster .get ("user_monster_id",0 ))
    if fresh_monster is None :
        return action_result (False ,"user_monster_id",monster .get ("user_monster_id",0 ),with_properties =True )
    ...
    return result ,{"monster_updates":[_paironormal_mode_update (fresh_monster ,mode ,mode_index ,player_object )]}
...
def _collect_paironormal_mode (username ,params ,mode_unique_id ):
    ...
    now =int (time .time ()*1000 )
    root ,player_object =load_player (username )
    island ,monster ,mode ,mode_index =_find_paironormal_mode_owner (player_object ,mode_unique_id )
    if mode is None :
        return action_result (False ,"user_monster_id",mode_unique_id ,with_properties =True )
    owed =float (mode .get ("collected_starpower",0.0 )or 0.0 )
    payout =max (0 ,int (owed ))
    if owed <=0 :
        result ={"star":0 ,"success":False ,
    ...
    mode ["last_collection"]=now
    save_player (username ,root )
    result ={"star":payout ,"success":True ,"user_monster_id":SFSLong (mode_unique_id )}
    return result ,{"monster_updates":[_paironormal_mode_update_legacy (mode ,mode_index )]}
def collect_monster (username ,params ):
    ...
    if find_monster (active_island ,monster_id )is None and monster_id !=-1 :
        _ ,_owner_monster ,mode ,_mode_index =_find_paironormal_mode_owner (player_object ,monster_id )
        if mode is not None :
    ...
    payout =max (_monster_stored_coins (monster ,max_coins ,income_rate ),_stored_collection_amount (monster ,collection_type ))
    player_key =_collection_player_key (collection_type )
    result_key =_collection_result_key (collection_type )
    if payout <=0 :
        result ={
        result_key :_collection_result_value (collection_type ,0 ),"success":False ,
        "message":"Normal monster: nothing to collect",
        "user_monster_id":SFSLong (monster_id ),
        }
        return result ,{}
    ...
    save_player (username ,root )
    result ={result_key :_collection_result_value (collection_type ,payout ),"success":True ,"user_monster_id":SFSLong (monster_id )}
...
def collect_multi_monster (username ,params ):
    ...
    island =find_island (player_object ,get_active_island_id (player_object ))
    if island is None :
        return action_result (False ,"user_island_id",get_active_island_id (player_object ),with_properties =True ),{}
    island_type =island_type_of (island )or 1
    monster_collections =[]
    update_monster_list =[]
    for monster in island .get ("monsters")or []:
        ...
        if island_type ==PAIRONORMAL_ISLAND_TYPE and monster .get ("modes"):
            ...
            monster_collections .append ({"star":mode_payout ,"user_monster_id":SFSLong (mid )})
            update_monster_list .append (_paironormal_mode_update (monster ,mode ,mode_index ))
            continue
        collection_type ,max_coins ,income_rate =compute_monster_economy (monster ,island_type )
        payout =max (_monster_stored_coins (monster ,max_coins ,income_rate ),_stored_collection_amount (monster ,collection_type ))
        if payout >0 :
            ...
            monster ["last_collection"]=now
            mid =monster .get ("user_monster_id",0 )
            monster_collections .append ({result_key :_collection_result_value (collection_type ,payout ),"user_monster_id":SFSLong (mid )})
            update_monster_list .append (_monster_update (monster ,"collect"))
    island ["last_collect_all"]=now
    save_player (username ,root )
    result ={
    "success":True ,"monster_collections":monster_collections ,
    "update_monster_list":update_monster_list ,
    "island":SFSLong (island .get ("user_island_id",0 )),"last_collect_all":SFSLong (now ),
    "properties":create_player_properties (player_object ),
    }
...
def extract_egg_costumes (username ,params ):
    ...
    save_player (username ,root )
    return {
    "egg":SFSLong (user_egg_id ),"success":True ,
    "island":requested_island_type or island_type_of (island ),
    "rewards":{
    "updateCostumes":True ,
    "loot":[{"amount":1 ,"premium":False ,"scaled":False ,"scale":False ,"id":costume_id ,"type":13 }],
...
def buy_egg (username ,params ):
    ...
    if requested_extra .get ("minor"):
        ...
        requested_common_name =(definition .get ("common_name")or "").lower ()if definition else ""
    ...
    if requires_direct_placement (get_monster_definition (monster_id ),island_type ):
        ...
        "user_egg_id":SFSLong (next_egg_id ),"costume":{"eq":0 ,"p":[]},
        "book_value":book_value ,"source":"direct_placement",
        }
    ...
    paironormal_parent =(resolve_paironormal_stored_monster_id (monster_id ,island_type )
    if is_paironormal_island (island_type )else monster_id )
    if is_paironormal_island (island_type )and is_paironormal_multimodal (get_monster_definition (paironormal_parent )):
        if requested_paironormal_mode is not None :
...
def _default_titansoul_state ():
    ...
    "can_awaken":False ,"rewards":[],"next_reward_time":0 ,"awakened_state":0 ,
    }
...
def _build_hatched_monster (monster_id ,island ,island_type ,user_monster_id ,pos_x ,pos_y ,flip ,now ,in_hotel =0 ,island_mode_override =None ,player_object =None ,reawakened_name =None ):
    definition =get_monster_definition (monster_id )
    island_uid =island .get ("user_island_id",1000 +island_type )
    spawn_level =1
    ...
    if is_box_monster_entity (definition )and island_type in BOX_SIMPLE_ISLAND_TYPES :
        ...
        "level":spawn_level ,"happiness":0 ,"times_fed":0 ,
    ...
    elif island_type ==AMBER_ISLAND_TYPE and not reawakened_name and is_box_monster_entity (definition ):
        monster =build_placeholder_box_monster (monster_id ,definition ,island_uid ,user_monster_id ,pos_x ,pos_y ,flip ,now ,island_type ,force_locked =True ,level =spawn_level )
    elif is_box_monster_entity (definition )and not reawakened_name :
        monster =build_placeholder_box_monster (monster_id ,definition ,island_uid ,user_monster_id ,pos_x ,pos_y ,flip ,now ,island_type ,level =spawn_level )
    else :
        monster ={
        "user_monster_id":SFSLong (user_monster_id ),
        "island":SFSLong (island_uid ),"monster":monster_id ,
...
def hatch_egg (username ,params ):
    ...
    nursery =None
    requested_paironormal_mode =None
    reawakened_name =matched_egg .get ("previous_name")if matched_egg is not None else None
    ...
    if matched_egg is None :
        ...
        (requires_direct_placement (candidate_definition ,island_type )or is_box_monster_entity (candidate_definition ))):
            monster_id =direct_monster_id

            for pending_egg in list (eggs ):
                if (pending_egg is not None and pending_egg .get ("monster")==monster_id
                and (pending_egg .get ("previous_name")or pending_egg .get ("source")=="wake_wubbox")):
                    reawakened_name =(pending_egg .get ("previous_name")
                    or (get_monster_definition (monster_id )or {}).get ("common_name")or "?")
                    pending_nursery_id =pending_egg .get ("structure",0 )
                    for structure in island .get ("structures")or []:
                        if structure is not None and structure .get ("user_structure_id")==pending_nursery_id :
        ...
        if nursery is not None and is_nursery_structure (nursery .get ("structure",nursery .get ("structure_id",0 ))):
            ...
            if island .get ("synthesizing"):
                for synth in island .get ("synthesizing")or []:
                    if synth is not None and synth .get ("structure")==nursery_id :
                        ...
                        break
                island ["synthesizing"]=[s for s in island .get ("synthesizing")or []
            ...
            if used_synth_monster :
                for i in range (len (island .get ("monsters")or [])-1 ,-1 ,-1 ):
                    ...
                    if source_monster is not None and source_monster .get ("user_monster_id")==used_synth_monster :
    ...
    island_mode_override =requested_paironormal_mode ,player_object =player_object ,reawakened_name =reawakened_name )
    ...
    import mod_api
    mod_api .fire_event ("player_hatch",username =username ,player_object =player_object ,
    monster =monster ,island =island )
    mod_api .fire_event ("egg_hatched",username =username ,player_object =player_object ,
    monster =monster ,island =island ,egg =matched_egg or {"user_egg_id":user_egg_id })
    xp_gain =definition .get ("xp",0 )or 0
    if xp_gain >0 :
        player_object ["xp"]=(player_object .get ("xp",0 )or 0 )+xp_gain
    import msm_composer
    composer_track =(msm_composer .track_for_monster (player_object ,island ,user_monster_id )
    if msm_composer .is_composer_island (island )else None )
    properties =create_player_properties (player_object )
    save_player (username ,root )
    result ={
    "success":True ,"create_in_storage":False ,"directPlace":nursery is None ,
    "user_egg_id":SFSLong (consumed_egg_id or user_egg_id ),"island":SFSLong (island_uid ),
    "properties":properties ,
    "monster":monster ,
    "monster_happy_effects":happy_effects ,
    }
...
def sell_egg (username ,params ):
    ...
    if msm_toggles .is_enabled ("functioning_currencies"):
        egg_monster_id =matched_egg .get ("monster")or matched_egg .get ("monster_id")or 0
        book_value =matched_egg .get ("book_value")or (get_monster_definition (egg_monster_id )or {}).get ("cost_coins",0 )or 0
        refund =_sell_refund (book_value )
        if refund >0 :
            player_object ["coins"]=(player_object .get ("coins",0 )or 0 )+refund
    eggs .remove (matched_egg )
    ...
    result ["success"]=True
    result ["properties"]=create_player_properties (player_object )
    nursery_update =_nursery_touch_payload (cleared_nursery ,player_object )if cleared_nursery is not None else {}
    return result ,nursery_update
def speed_up_hatching (username ,params ):
    ...
    if island is not None and egg is not None :
        ...
        result ["properties"]=create_player_properties (player_object )
        add_actual_currencies (result ,player_object )
        nursery_update =_nursery_touch_payload (nursery ,player_object )if nursery is not None and not _is_synthesizer_structure (nursery )else {}
        return result ,nursery_update
    return result ,{}
...
def costume_action (username ,params ,command ):
    ...
    root ,player_object =load_player (username )
    dirty =False
    if command =="purchase_costume"and msm_toggles .is_enabled ("functioning_currencies"):
        ...
        if cost_diamonds >0 :
            ...
            player_object ["medals"]=max (0 ,(player_object .get ("medals",0 )or 0 )-cost_medals )
            dirty =True
    if monster_id :
...
def breed_monsters (username ,params ):
    ...
    for existing in breeding_list :
        if existing is not None and existing .get ("structure")==structure_sid :
            result =action_result (False ,"user_structure_id",structure_id ,with_properties =True )
            result ["error"]="BREEDING_STRUCTURE_BUSY"
            if process_refs_repaired :
                save_player (username ,root )
            return result
    monster_id =choose_breeding_result_monster (parent_a .get ("monster",0 ),parent_b .get ("monster",0 ),island_type_of (island )or 0 )
    now =int (time .time ()*1000 )
    build_ms =_effective_build_ms (monster_id )
    ...
    properties =msm_rewardtracks .properties_with_encore (player_object ,2 ,points ,action_kind ="breed")
    save_player (username ,root )
    return {
...
def finish_breeding (username ,params ,force_complete =False ):
    ...
    if not monster_id :
        ...
        monster_id =choose_breeding_result_monster (parent_a_monster ,parent_b_monster ,island_type_of (island )or 0 )
    if not monster_id :
        if process_refs_repaired :
            save_player (username ,root )
        return action_result (False ,"user_breeding_id",breeding_id ,with_properties =True )
    build_ms =_effective_build_ms (monster_id )
    speedup =bool (params .get ("speedup")or params .get ("speed_up")or params .get ("speedUp"))
    user_egg ,nursery =place_egg (player_object ,island ,monster_id ,"finish_breeding",ready =speedup )
    if user_egg is None or nursery is None :
        result =action_result (False ,"user_breeding_id",breeding_id ,with_properties =True )
...
def move_battle_monster (username ,params ,send_home ):
    ...
    nursery =next ((s for s in target_island .get ("structures")or []
    if s and (get_structure_definition (s .get ("structure",0 ))or {}).get ("structure_type")=="nursery"),None )
...
def _complete_egg_teleport (username ,root ,source_island ,target_island ,monster ,user_egg ,nursery ):
    source_island ["monsters"]=[m for m in (source_island .get ("monsters")or [])if m is not monster ]
    island_uid =target_island .get ("user_island_id",0 )
    nursery_sid =nursery .get ("user_structure_id",0 )
    user_egg ["structure"]=SFSLong (nursery_sid )
    user_egg ["island"]=SFSLong (island_uid )
    for key in ("monster_id","book_value","source","nursery_id","user_structure_id","user_island_id"):
        user_egg .pop (key ,None )
    nursery .pop ("finishing_time",None )
    nursery ["building_completed"]=_safe_int (nursery .get ("date_created"),int (time .time ()*1000 ))
    save_player (username ,root )
    return {
    "success":True ,"user_monster_id":SFSLong (monster .get ("user_monster_id",0 )),
    "sent_to_island":island_type_of (target_island ),
    "dest_nursery":SFSLong (nursery_sid ),
    "user_island_id":SFSLong (island_uid ),"user_egg":user_egg ,
    },nursery
...
def teleport_monster_to_island (username ,params ):
    ...
    user_egg ,nursery =place_egg (player_object ,target_island ,resolved_type ,"teleport",
    previous_name =previous_name )
    if user_egg is None or nursery is None :
        return {"success":False ,"error":"NO_AVAILABLE_EGG_HOLDER"},None

    return _complete_egg_teleport (username ,root ,source_island ,target_island ,monster ,user_egg ,nursery )
...
def send_to_magical_nexus (username ,params ):
    ...
    monster_type =monster .get ("monster",0 )
    previous_name =monster .get ("name")
    user_egg ,nursery =place_egg (player_object ,target_island ,monster_type ,"magical_nexus",previous_name =previous_name )
    if user_egg is None or nursery is None :
        return {"success":False ,"error":"NO_AVAILABLE_EGG_HOLDER"},None
    return _complete_egg_teleport (username ,root ,source_island ,target_island ,monster ,user_egg ,nursery )
...
def send_to_paironormal (username ,params ):
    ...
    user_egg ,nursery =place_egg (player_object ,target_island ,monster_type ,"paironormal",modes =egg_modes )
    if user_egg is None or nursery is None :
        return {"success":False ,"error":"NO_AVAILABLE_EGG_HOLDER"},None
    if arriving_mode is not None :
...
def place_on_gold_island (username ,params ):
    ...
    return {"success":True ,"user_monster_id":SFSLong (monster_id ),"monster":wire_monster }
...
def store_buddy (username ,params ):
    ...
    stored [:]=[b for b in stored if b is not None and b .get ("user_structure_id")!=structure_id ]
    stored .append ({
...
def unstore_buddy (username ,params ):
    ...
    entry =next ((b for b in stored if b is not None and b .get ("user_structure_id")==structure_id ),None )
    if entry is None :
        return {"success":False ,"user_structure_id":SFSLong (structure_id )},{}
    ...
    if isinstance (record ,dict ):
        ...
        for key ,value in (("colorR",entry .get ("colorR")),("colorY",entry .get ("colorY")),("colorB",entry .get ("colorB"))):
            if value is not None :
                structure [key ]=value
        already =any (
...
def start_fuzing (username ,params ):
    ...
    if not create and not color_r and not color_y and not color_b :
        ...
        if fuzer_inputs :
            color_r =sum (b .get ("colorR",0.0 )or 0.0 for b in fuzer_inputs )/len (fuzer_inputs )
            color_y =sum (b .get ("colorY",0.0 )or 0.0 for b in fuzer_inputs )/len (fuzer_inputs )
            color_b =sum (b .get ("colorB",0.0 )or 0.0 for b in fuzer_inputs )/len (fuzer_inputs )
    ...
    if not msm_toggles .is_enabled ("instant_timers"):
        build_ms =int ((get_structure_definition (FUZER_BUDDY_STRUCTURE_ID )or {}).get ("build_time",7200 )or 7200 )*1000
    finished_on =now +build_ms
    record ={
    "structure_id":SFSLong (structure_sid ),"started_on":SFSLong (now ),"finished_on":SFSLong (finished_on ),
    "create":create ,"colorR":color_r ,"colorY":color_y ,"colorB":color_b ,
    }
    fuzer_list .append (record )
    save_player (username ,root )
    return {
    "success":True ,"structure_id":SFSLong (structure_sid ),"user_fuzing":record ,
    "properties":create_player_properties (player_object ),
    }
def speed_up_fuzing (username ,params ):
    ...
    save_player (username ,root )
    result ={
    "success":True ,"started_on":record .get ("started_on"),"finished_on":SFSLong (finished_on ),
    "structure_id":SFSLong (structure_id ),"properties":create_player_properties (player_object ),
    }
    return result ,{}
def finish_fuzing (username ,params ,force_complete =False ):
    ...
    if not force_complete and finished_on >now :
        result =action_result (False ,"structure_id",structure_id ,with_properties =True )
    ...
    if fuzer_inputs and not record .get ("create"):
        ...
        color_y =sum (b .get ("colorY",0.0 )or 0.0 for b in fuzer_inputs )/len (fuzer_inputs )
        color_b =sum (b .get ("colorB",0.0 )or 0.0 for b in fuzer_inputs )/len (fuzer_inputs )
        delete_ids =[{"id":SFSLong (b .get ("user_structure_id",0 ))}for b in fuzer_inputs ]
        consumed ={b .get ("user_structure_id")for b in fuzer_inputs }
        island ["structures"]=[
        s for s in island .get ("structures")or []
    ...
    "settings":43690 ,
    "colorB":color_b ,
    "island":SFSLong (island_uid ),"user_island_id":island_uid ,
...
def start_amber_evolve (username ,params ):
    ...
    evolve_list [:]=[r for r in evolve_list if r is not None and r .get ("structure_id")!=structure_id ]
    evolve_list .append ({
    "structure_id":SFSLong (structure_id ),"user_monster_id":SFSLong (user_monster_id ),
    "started_on":SFSLong (now ),"finished_on":SFSLong (finished_on ),
    "from_monster":monster .get ("monster",0 )or 0 ,"to_monster":next_monster_id ,
    })
    save_player (username ,root )
    import msm_islands
    crucible =msm_islands .crucible_wire_object_for_island (island )
    result ={
    "success":True ,"last_user_monster_id":SFSLong (user_monster_id ),
...
def finish_amber_evolve (username ,params ,force_complete =False ):
    ...
    def _not_ready (user_monster_id =0 ):
        ...
        if crucible_island is None :
            crucible_island =find_island (player_object ,get_active_island_id (player_object ))if user_monster_id else None
        crucible =msm_islands .crucible_wire_object_for_island (crucible_island )
        payload ={
        "success":False ,"verify":False ,"user_structure_id":SFSLong (structure_id ),
        "user_monster_id":SFSLong (user_monster_id ),"evolve_success":False ,
        "new_monst":0 ,"mercy_flag":"",
        }
        if crucible is not None :
    ...
    if monster is not None :
        ...
        if next_monster_id >0 and monster .get ("monster")!=next_monster_id :
            previous_name =definition .get ("common_name")or definition .get ("name")or ""
            ...
            if not monster .get ("name")or monster .get ("name")==previous_name :
                monster ["name"]=definition .get ("common_name")or definition .get ("name")or ""
    ...
    crucible =msm_islands .crucible_wire_object_for_island (island ,{
    "monster":user_monster_id ,"new_type":next_monster_id if evolved else 0 ,
    "e":0 ,"started_on":0 ,"complete_on":0 ,
    })
    result ={
    "success":True ,"verify":False ,"user_structure_id":SFSLong (structure_id ),
    "user_monster_id":SFSLong (user_monster_id ),"evolve_success":evolved ,
    "new_monst":next_monster_id if evolved else 0 ,"mercy_flag":"",
    }
    if crucible is not None :
        result ["user_crucible"]=crucible
    if evolved :
