...
_WIRE_LONG_KEYS = {
    ...
    "underling_id",
    "user_underling_id",
    "selectedUnderlingUid",
    "parent_monster",
    "parent_user_monster_id",
    "parent_island",
    "parent_user_island_id",
    "active_island",
    "last_user_monster_id",
    "last_user_egg_id",
    "last_user_island_id",
    "last_collection",
    "last_collected",
...
def _decode_wire_bits(value):
    ...
    if hex_bits is not None:
        try:
            return float(
                struct.unpack(">d", bytes.fromhex(str(hex_bits).removeprefix("0x")))[0]
            )
    ...
    if isinstance(value, dict):
        ...
        for key, sub in value.items():
            if isinstance(sub, dict) or isinstance(sub, list):
                ...
                key in _WIRE_LONG_KEYS
                and isinstance(sub, int)
...
PROPERTY_ALIASES = [
    ...
    ("minigame_tokens", "minigame_tokens_actual"),
    ("starpower", "starpower_actual"),
    ("sticker_stars", "sticker_stars_actual"),
]
...
def create_player_properties(player_object):
    ...
    for source_key, actual_key in PROPERTY_ALIASES:
        value = _currency_value(
            player_object.get(source_key, player_object.get(actual_key, 0)) or 0
        )
    ...
    properties.append(
        {"daily_bonus_type": player_object.get("daily_bonus_type") or "none"}
    )
    properties.append(
        {
            "daily_bonus_amount": _clamp_i32(
                player_object.get("daily_bonus_amount", 0) or 0
            )
        }
    )

    properties.append(
        {"reward_day": _clamp_i32(player_object.get("reward_day", 0) or 0)}
    )
    properties.append(
        {"has_free_ad_scratch": bool(player_object.get("has_free_ad_scratch", False))}
    )
    properties.append(
        {
            "daily_relic_purchase_count": _clamp_i32(
                player_object.get("daily_relic_purchase_count", 0) or 0
            )
        }
    )
    properties.append(
        {
            "relic_diamond_cost": _clamp_i32(
                player_object.get("relic_diamond_cost", 1) or 1
            )
        }
    )
    ...
    properties.append(
        {
            "earned_starpower": _currency_value(
    ...
    properties.append(
        {
            "speed_up_credit": _currency_value(
                player_object.get("speed_up_credit", 0) or 0
            )
        }
    )
    properties.append(
        {"battle_xp": _currency_value(player_object.get("battle_xp", 0) or 0)}
    )
    properties.append(
        {"battle_level": _clamp_i32(player_object.get("battle_level", 0) or 0)}
    )
    ...
    if isinstance(player_object.get("currency_level_scale"), dict):
        properties.append(
            {"currency_level_scale": player_object.get("currency_level_scale")}
        )

    if "encore_points" in player_object:
        properties.append(
            {"encore_points": SFSFloat(player_object.get("encore_points", 0.0) or 0.0)}
        )
    if "encore_level" in player_object:
        properties.append(
            {"encore_level": _clamp_i32(player_object.get("encore_level", 1) or 1)}
        )
    if "encore_tutorial" in player_object:
        properties.append(
            {"encore_tutorial": bool(player_object.get("encore_tutorial", False))}
        )
    return properties
