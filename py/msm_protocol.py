...
def _normalize_wire_value(value):
...
def _encode_utf(value):
    ...
    if len(data) > 65535:
        raise ValueError(f"SFS UTF string is too long: {len (data )} bytes")
...
def _encode_object_body(data):
    ...
    for key, value in data.items():
        value = _normalize_wire_value(value)
        sfs_type = _infer_type(value)
        body += (
            _encode_utf(key)
            + struct.pack(">B", sfs_type)
            + _encode_value(sfs_type, value)
        )
...
class _Reader:
    ...
    def read(self, n):
        ...
        if len(chunk) != n:
            raise ValueError(
                f"expected {n } bytes at offset {self .pos }, got {len (chunk )}"
            )
        self.pos += n
        return chunk
...
def _decode_value(reader, sfs_type):
    ...
    if sfs_type == SFS_TYPE_SHORT_ARRAY:
        return [
            struct.unpack(">h", reader.read(2))[0] for _ in range(reader.read_u16())
        ]
    if sfs_type == SFS_TYPE_INT_ARRAY:
        return [
            struct.unpack(">i", reader.read(4))[0] for _ in range(reader.read_u16())
        ]
    if sfs_type == SFS_TYPE_LONG_ARRAY:
        return [
            struct.unpack(">q", reader.read(8))[0] for _ in range(reader.read_u16())
        ]
    if sfs_type == SFS_TYPE_FLOAT_ARRAY:
        return [
            struct.unpack(">f", reader.read(4))[0] for _ in range(reader.read_u16())
        ]
    if sfs_type == SFS_TYPE_DOUBLE_ARRAY:
        return [
            struct.unpack(">d", reader.read(8))[0] for _ in range(reader.read_u16())
        ]
    if sfs_type == SFS_TYPE_UTF_STRING_ARRAY:
