...
from msm_playerdata import create_player_properties ,find_island ,island_type_of ,load_player ,save_player
...
def _islands_matching (player_object ,on_island ):
    ...
    return [isl for isl in (player_object .get ("islands")or [])
...
def _structure_ids (structure ):
    if not isinstance (structure ,dict ):
        return set ()
    structure_id =_safe_int (structure .get ("structure")or structure .get ("structure_id"))
    if not structure_id :
        return set ()
...
def _soul_link_count (state ,rarity ):
    ...
    for link in links :
        if isinstance (link ,dict ):
            link_id =_safe_int (link .get ("monster")or link .get ("monster_id")or link .get ("id"))
        else :
            link_id =_safe_int (link )
        if not link_id :
            continue
        name =((get_monster_definition (link_id )or {}).get ("common_name")or "").upper ()
        if rarity =="epic":
...
def _titansoul_level_rows ():
    ...
    if _titansoul_levels_cache is None :
        ...
        rows =[r for r in (data .get ("titansoul_level_data")or [])if isinstance (r ,dict )]
        _titansoul_levels_cache =sorted (rows ,key =lambda r :_safe_int (r .get ("min_links")))
    return _titansoul_levels_cache

def _titansoul_song_parts (state ):
    links =state .get ("soul_links")
    num_links =len (links )if isinstance (links ,list )else _safe_int (state .get ("num_links"))
    song_part =0
...
def _titansoul_has_reward (state ,reward_type ):
    ...
    for reward in rewards :
        if isinstance (reward ,dict ):
            name =str (reward .get ("type")or reward .get ("reward")or reward .get ("name")or "").lower ()
        else :
...
def _goal_satisfied (player_object ,goal ):
    ...
    if "island"in goal and "on_island"not in goal :
        ...
        return any (island_type_of (isl )==target for isl in (player_object .get ("islands")or [])if isl is not None )
    ...
    if "activate_box"in goal :
        for isl in islands :
            for monster in (isl .get ("monsters")or []):
                ...
                if is_box_monster_entity (definition )and is_awakened_box_monster (monster ):
                    return True
        return False
        ...
        for isl in islands :
            for record in (isl .get ("breeding")or []):
                if not isinstance (record ,dict ):
                    continue
                if (_safe_int (record .get ("monster_1"))==left_id
                and _safe_int (record .get ("monster_2"))==right_id ):
                    return True
        return False

    if "start_clubbox"in goal or "go_to_clubbox"in goal :
        return bool (player_object .get ("clubboxes")or _safe_int (player_object .get ("last_clubbox"))>0 )
...
def advance_quests (username ):
    ...
    for qid_str ,entry in list (state .items ()):
        ...
        if not flags :
            continue
        status_str =_status_string (flags ,completed )
        if status_str ==entry .get ("status")and completed ==bool (entry .get ("completed")):
            continue
        entry ["status"]=status_str
        entry ["completed"]=completed
        dirty =True
        changed_results .append ({"quest_id":qid ,"status":status_str })
        if completed :
            changed_results .extend (_unlocked_children (defs ,parents ,state ,qid ,user_id ))
...
def _unlocked_children (defs ,parents ,state ,completed_qid ,user_id ):
    ...
    if not completed_def :
        return extra
    child_names ={nxt ["quest"]for nxt in (completed_def .get ("next")or [])if isinstance (nxt ,dict )and nxt .get ("quest")}
    if not child_names :
        return extra
    by_name ={definition .get ("name"):(qid ,definition )for qid ,definition in defs .items ()}
    for child_name in child_names :
        ...
        seq_id =len (state )
        extra .append ({"new":[_new_record (child_def ,child_qid ,seq_id ,user_id ),child_def ]})
    return extra

_QUEST_REWARD_ACTUAL_KEYS ={
"coins":"coins_actual","diamonds":"diamonds_actual","relics":"relics_actual",
"ethereal_currency":"ethereal_currency_actual","food":"food_actual",
"starpower":"starpower_actual","xp":"xp",
}
...
def _collect (username ,root ,player_object ,defs ,state ,quest_id ):
    ...
    if not entry .get ("collected"):
        ...
        if starpower :
            player_object ["starpower"]=_safe_int (player_object .get ("starpower"))+starpower
    ...
    changed_properties =[full_properties [k ]for k in actual_keys if k in full_properties ]
    return {"result":[{"collect":quest_id }]},changed_properties

def _report_status (username ,root ,player_object ,defs ,parents ,state ,meta ,user_id ,quest_id ,status_param ):

    status_str =status_param if isinstance (status_param ,str )else json .dumps (status_param )
    entry =state .setdefault (str (quest_id ),{"delivered":True })
    entry ["status"]=status_str
...
def _poll (username ,root ,player_object ,defs ,parents ,state ,meta ,user_id ):
    ...
    if not meta .get ("catalog_sent"):
        return _full_catalog (username ,root ,player_object ,defs ,parents ,state ,meta ,user_id )
    ...
    for qid ,definition in defs .items ():
        ...
        seq_id =len (state )
        result .append ({"new":[_new_record (definition ,qid ,seq_id ,user_id ,entry ),definition ]})
        changed =True
    if changed :
        meta ["event_id"]=_safe_int (meta .get ("event_id"))+1
        save_player (username ,root )
...
def gs_quest (username ,params ):
    ...
    if quest_id is not None and status_param is not None :
        return _report_status (username ,root ,player_object ,defs ,parents ,state ,meta ,user_id ,_safe_int (quest_id ),status_param )

    for collect_key in ("collect","quest_claim_id","quest_id","questId","id","claim","claim_id","quest"):
        if collect_key in params :
            return _collect (username ,root ,player_object ,defs ,state ,_safe_int (params .get (collect_key )))
    ...
    if isinstance (batch ,list )and batch :
        ...
        for item in batch :
            ...
            if item_qid is None or item_status is None :
                continue
            last_response =_report_status (username ,root ,player_object ,defs ,parents ,state ,meta ,user_id ,_safe_int (item_qid ),item_status )
...
def gs_quest_read (username ,params ):
    ...
    targets =ids if ids else ([quest_id ]if quest_id is not None else [k for k in state .keys ()if not k .startswith ("_")])
    for qid in targets :
