...
def _pick_wheel_prize(prizes):
    ...
    if prize is not None and str(prize.get("prize", "")) == "jackpot":
        bonus_pool = [
            p
            for p in prizes
            if p.get("is_top_prize", 0) == 1 and str(p.get("prize", "")) != "jackpot"
        ]
...
def _scaled_prizes_payload(prizes):
    ...
    for prize_id in _SCALED_PRIZE_IDS:
        ...
        if prize is not None:
            rows.append(
                {"id": prize_id, "scaled_amount": int(prize.get("amount", 0) or 0)}
            )
    return {"prizes": rows}
...
_SCRATCH_PRICE_SETTING = {
    "M": "USER_MONSTER_SCRATCHOFF_PRICE",
    "S": "USER_SCRATCHOFF_PRICE",
}
...
def play_scratch_off(username, params):
    ...
    if is_purchase:
        ...
        price = get_user_game_setting_int(
            _SCRATCH_PRICE_SETTING.get(request_type, "USER_SCRATCHOFF_PRICE"), 0
        )
        if price > 0:
    ...
    prizes = (
        get_spin_wheel_prizes()
        if request_type == "S"
        else get_scratch_prizes(request_type)
    )
    if request_type == "M":
    ...
    if is_purchase:
        ...
        return [
            ...
            (
                ...
                {"properties": [diamonds_property] if diamonds_property else []},
            ),
        ]
...
def collect_scratch_off(username, params):
    ...
    if ticket is None:
        ...
        if (
            ...
            and str(legacy.get("type") or "").upper() == request_type
        ):
            ticket = legacy
    ticket = ticket or {}
    ...
    if prize_type != "monster":
        ...
        island = None
        if definition and requires_direct_placement(
            definition, island_type_of(active_island) if active_island else 0
        ):
        ...
        if island is not None:
            user_egg, _nursery = place_egg(
                player_object, island, amount, "scratch_off", ready=True
            )
            if user_egg is None and requires_direct_placement(
                definition, island_type_of(island)
            ):
                ...
                user_egg = {
                    "monster": amount,
                    "monster_id": amount,
                    "laid_on": SFSLong(now_ms),
                    "hatches_on": SFSLong(now_ms),
                    "structure": SFSLong(0),
    ...
    properties.append(
        {
            _scratch_flag_key(request_type): bool(
                player_object.get(_scratch_flag_key(request_type), False)
            )
        }
    )
...
def _flip_prize_pool():
    prizes = [
        p for p in get_spin_wheel_prizes() if p.get("prize") in _FLIP_SAFE_PRIZE_TYPES
    ]
...
def flip_minigame_cost(username, params):
    ...
    for pool_id in (2, 3, 4):
        prizes = [
            p
            for p in _flip_prize_pool()
            if p.get("is_top_prize", 0) == (1 if pool_id == 4 else 0)
        ]
        if prizes:
            pools.append(
                {
...
def purchase_flip_mini_game(username, params):
    ...
    reward = {
        "pool": 1,
        "amt": int(prize.get("amount", 0) or 0),
        "type": prize.get("prize", "coins"),
    }
...
def purchase_memory_mini_game(username, params):
    ...
    if msm_toggles.is_enabled("functioning_currencies"):
        player_object["coins"] = max(
            0, _safe_int(player_object.get("coins", 0)) - coin_cost
        )
