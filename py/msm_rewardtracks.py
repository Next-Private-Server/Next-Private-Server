...
def _wire_loot_item(item):
    ...
    if resource == "card_pack":
        ...
        return {
            ...
            "scale": False,
            "id": pack_type,
            "type": _LOOT_WIRE_TYPES["card_pack"],
        ...
        "data": [
            {
                "rolls_over": True,
                "encore_type": 0,
                "reward_track_id": _ENCORE_TRACK_ID,
                "label": "Encore - Breeding",
            }
        ],
        "id": _ENCORE_EVENT_ID,
        "start_date": start,
...
def _sync_encore_player_fields(player_object, track_id, total_points, track=None):
    ...
    player_object["encore_level"] = _current_level_number(
        track, _claimed_ids(player_object)
    )
...
def encore_progress(player_object, track_id, points, action_kind=None):
    ...
    if all_items:
        entry["loot"] = [
            _wire_loot_item(it)
            for it in all_items
            if it.get("resource") in _LOOT_WIRE_TYPES
        ]
    ...
    entry["current_encore_state"] = {
        "event_id": event_id,
        "total_points": SFSFloat(after),
    }
...
def _apply_loot(player_object, loot_json):
    ...
    for key, amount in loot.items():
        ...
        if key == "card_pack" or key.startswith("card_pack."):
            ...
            items.append(
                {"amount": amount, "resource": "card_pack", "pack_type": pack_type}
            )
        elif key == "monster":
            ...
            and str(amount.get("type", "")) == "MONSTER"
        ):
