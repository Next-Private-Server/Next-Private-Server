    ...
    if isinstance(value, list):
        ...
        for entry in value:
            if (
                ...
                and entry.get(id_key) in existing_ids
            ):
                continue
        ...
        for db_file in sorted(db_files_dir.glob("*.json")):
            ...
            try:
                ...
                if not isinstance(mod_data, dict):
                    logger.warning(
                        "db file %s: top-level JSON must be an object", db_file
                    )
                    continue
                ...
                if isinstance(existing, dict):
                    ...
                    logger.info(
                        "db file %s: merged into existing %s (%d item(s) added/updated)",
                        db_file,
                        db_name,
                        added,
                    )
...
_FROZEN_CURRENCY_KEYS = frozenset(
    _FROZEN_CURRENCY_BASE_KEYS
    + tuple(f"{key }_actual" for key in _FROZEN_CURRENCY_BASE_KEYS)
)
...
def _fallback_player_file(missing_path):
    ...
    candidates = [
        ...
        if p.is_file()
        and not any(
            mark in p.name.lower()
            for mark in (".bak", "backup", "previous", "template")
        )
    ]
    if not candidates:
        return None
...
def load_user_data(username):
    ...
    if not msm_toggles.is_enabled("functioning_currencies") and isinstance(
        root.get("player_object"), dict
    ):
