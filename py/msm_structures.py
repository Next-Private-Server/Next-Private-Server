...
build_paironormal_modes ,find_paironormal_form_id ,get_bakery_food ,get_ethereal_islet_definition ,
get_monster_definition ,get_monster_level_definition ,get_structure_definition ,
get_user_game_setting_int ,resolve_paironormal_stored_monster_id ,_paironormal_starpower_owed ,
)
...
def _obstacle_user_structure_snapshot (structure ):
    return {
    "island":SFSLong (structure .get ("island",structure .get ("user_island_id",0 ))or 0 ),
...
def _structure_update (structure ,mode ="full"):
    ...
    if mode =="move":
        ...
        "muted":bool (structure .get ("muted",0 )),"in_warehouse":structure .get ("in_warehouse",0 ),
        "is_upgrading":structure .get ("is_upgrading",0 ),"is_complete":structure .get ("is_complete",1 ),
        "building_completed":SFSLong (structure .get ("building_completed",0 )or 0 ),
        "last_collection":SFSLong (structure .get ("last_collection",0 )or 0 ),
        "obj_data":structure .get ("obj_data",0 ),"obj_end":SFSLong (structure .get ("obj_end",0 )or 0 ),
...
def sell_structure (username ,params ):
    ...
    if island is not None :
        ...
        if msm_toggles .is_enabled ("functioning_currencies"):
            sold_structure =next (
            (s for s in structures if s is not None and s .get ("user_structure_id")==structure_id ),None ,
            )
            if sold_structure is not None :
                book_value =sold_structure .get ("book_value")or 0

                refund =int (book_value *(msm_toggles .get_int ("sell_percentage",75 ,minimum =0 ,maximum =100 )/100.0 ))
                if refund >0 :
        ...
        if eggs :
            island ["eggs"]=[e for e in eggs if e is None or e .get ("structure")!=structure_id ]
        breeding =island .get ("breeding")
        if breeding :
            island ["breeding"]=[b for b in breeding if b is None or b .get ("structure")!=structure_id ]
        synthesizing =island .get ("synthesizing")
        if synthesizing :
            island ["synthesizing"]=[s for s in synthesizing if s is None or s .get ("structure")!=structure_id ]
        import msm_monsters
        happy_effects =msm_monsters .recompute_island_happiness (island )
        save_player (username ,root )
    else :
...
def collect_nucleus_reward (username ,params ):
    structure_id =params .get ("user_structure_id",0 )or params .get ("structure_id",0 )or 0
    root ,player_object =load_player (username )
    island ,structure =_locate (player_object ,structure_id )
    if structure is None :
        return action_result (False ,"user_structure_id",structure_id ,with_properties =True )
    ...
    interval =get_user_game_setting_int ("USER_NUCLEUS_REWARD_INTERVAL",70298 )*1000 or 70298000
    nucleus .update ({
    ...
    result ["loot"]=[{
    "amount":amount ,"premium":False ,"scaled":False ,"scale":False ,"id":0 ,"type":5 ,
    }]
    result ["properties"]=create_player_properties (player_object )
    add_actual_currencies (result ,player_object )
    return result
...
def start_obstacle (username ,params ):
    ...
    if not _is_obstacle_definition (definition ):
        return action_result (False ,"user_structure_id",structure_id ,with_properties =True ),{}
    build_ms =INSTANT_TIMER_FLOOR_MS if msm_toggles .is_enabled ("instant_timers")else max (0 ,definition .get ("build_time",0 )or 0 )*1000
    now =int (time .time ()*1000 )
...
def clear_obstacle_speed_up (username ,params ):
    structure_id =(params .get ("user_structure_id",0 )or params .get ("structure_id",0 )
    or params .get ("userStructureId",0 )or 0 )
    root ,player_object =load_player (username )
    island ,structure =_locate (player_object ,structure_id )
    if structure is None :
        return action_result (False ,"user_structure_id",structure_id ,with_properties =True ),{}
...
"is_complete","is_upgrading","building_completed","date_created","finishing_time","obj_end",
}
_UPDATE_STRUCTURE_KEYS =_UPDATE_STRUCTURE_UPGRADE_START_KEYS |{
"pos_x","pos_y","col","row","scale","flip","flipped","muted","in_warehouse","obj_data",
"last_collection","last_collected","occupied","has_egg","viewed","level",
}
_LONG_STRUCTURE_KEYS ={"building_completed","date_created","finishing_time","obj_end","last_collection","last_collected"}
def update_structure (username ,params ):
    ...
    result ["properties"]=structure_properties +create_player_properties (player_object )
    return result
...
def start_upgrade_structure (username ,params ):
    ...
    if success :
        ...
        build_ms =INSTANT_TIMER_FLOOR_MS if msm_toggles .is_enabled ("instant_timers")else max (0 ,(definition .get ("build_time",0 )if definition else 0 ))*1000
        complete_on =now +build_ms
...
def speed_up_upgrade_structure (username ,params ):
    ...
    remaining =(structure .get ("building_completed",finished_at )or finished_at )-finished_at
    _charge_speedup (player_object ,remaining )
...
def _fugue_target_mode (player_object ,monster_id ,fallback_island ):
    ...
    stored =resolve_paironormal_stored_monster_id (mon .get ("monster",0 ),island_type )or mon .get ("monster",0 )
    ...
    has_major =any (isinstance (e ,dict )and e .get ("monster")==major_id and e .get ("a")for e in modes )
    has_minor =any (isinstance (e ,dict )and e .get ("monster")==minor_id and e .get ("a")for e in modes )
    if has_minor and not has_major :
        return 0
    if has_major and not has_minor :
        return 1
    current =(fallback_island .get ("mode",fallback_island .get ("island_mode",0 ))or 0 )if fallback_island else 0
    return 1 -current
...
def start_fuguing (username ,params ):
    ...
    if success :
        ...
        _fugue_monster_island ,fugue_monster =find_monster_with_island (player_object ,monster_id )
        species_id =(fugue_monster or {}).get ("monster",0 )or monster_id
        stored_id =resolve_paironormal_stored_monster_id (species_id ,island_type )or species_id
        ...
        if duration_ms <=0 :
            ...
            upgrade_id =definition .get ("upgrades_to")or _upgrade_target (structure .get ("structure",0 ))
    ...
    structure ["obj_end"]=complete_on
    fuguing_entries =[
    e for e in (island .get ("fuguing")or [])
...
def speed_up_fuguing (username ,params ):
    ...
    for entry in island .get ("fuguing")or []:
        if isinstance (entry ,dict )and entry .get ("structure")==structure_id :
            remaining =(entry .get ("complete_on",finished_at )or finished_at )-finished_at
            _charge_speedup (player_object ,remaining )
            break
    structure ["obj_end"]=finished_at
...
def finish_fuguing (username ,params ):
    ...
    player_object ["total_starpower_collected"]=(player_object .get ("total_starpower_collected",0 )or 0 )+reward

    fuguing_entries =(island .get ("fuguing")or [])if island else []
    pending =None
    remaining =[]
    for entry in fuguing_entries :
        if pending is None and isinstance (entry ,dict )and entry .get ("structure")==structure_id :
            pending =entry
            continue
    ...
    if pending is not None and pending .get ("success"):
        ...
        if monster_id :
            ...
            if monster is not None and island_type :
                new_entry =build_paironormal_modes (monster ,island_type ,activated_mode ,player_object )
                base_uid =monster .get ("user_monster_id")
                if new_entry and base_uid :
                    now =int (time .time ()*1000 )
                    stored_id =resolve_paironormal_stored_monster_id (monster .get ("monster",0 ),island_type )
                    ...
                    book_value =(
                    (get_monster_definition (major_id )or {}).get ("cost_coins",0 )or 0
                    )+(
                    (get_monster_definition (minor_id )or {}).get ("cost_coins",0 )or 0
                    )
                    monster ["book_value"]=book_value
                    monster ["happiness"]=100
                    starpower_owed =_paironormal_starpower_owed (
                    monster .get ("level",1 )or 1 ,monster .get ("last_collection",now )or now ,now ,
                    )
                    monster ["last_collection"]=now
                    filled_mode =0 if new_entry .get ("monster")==major_id else 1
                    extra_frames .append (("gs_update_monster",{
                    "mode":filled_mode ,"mode_state":new_entry ,"user_monster_id":SFSLong (base_uid ),
                    }))
                    extra_frames .append (("gs_update_monster",{
                    "book_value":book_value ,"happiness":100 ,"user_monster_id":SFSLong (base_uid ),
                    }))
                    extra_frames .append (("gs_update_monster",{
                    "mode":1 -filled_mode ,"user_monster_id":SFSLong (base_uid ),
                    "last_collection":now ,"collected_starpower":SFSFloat (starpower_owed ),
                    }))
    elif pending is not None :

        monster_id =pending .get ("monster")or 0
        if monster_id :
            _failed_island ,failed_monster =find_monster_with_island (player_object ,monster_id )
            if failed_monster is not None :
                import msm_monsters
                feed_update =msm_monsters .apply_feed (
                player_object ,failed_monster ,monster_id ,charge_food =False ,
                )
                if feed_update :
...
def viewed_fugued_monster (username ,params ):
    ...
    structure_id =params .get ("user_structure_id",0 )or 0
    root ,player_object =load_player (username )
    island ,structure =_locate (player_object ,structure_id )if structure_id else (None ,None )
    if structure is not None :
...
def merge_fugue (username ,params ):
    ...
    if island is None and requested_monster_id :
        for candidate_island in player_object .get ("islands")or []:
            ...
            for entry in candidate_island .get ("fuguing")or []:
                if isinstance (entry ,dict )and entry .get ("monster")==requested_monster_id :
    ...
    remaining =[]
    for entry in fuguing_entries :
        matches =isinstance (entry ,dict )and pending is None and (
        (structure_id and entry .get ("structure")==structure_id )
        or (requested_monster_id and entry .get ("monster")==requested_monster_id )
        )
        if matches :
            pending =entry
            continue
...
def buy_structure (username ,params ):
    ...
    if island is not None :
        ...
        save_player (username ,root )
    import msm_monsters
    happy_effects =msm_monsters .recompute_island_happiness (island )if island is not None else []
...
def update_awakener (username ,params ):
    ...
    if titansoul_id :
        ...
        import msm_monsters
        titansoul =monster .setdefault ("titansoul",msm_monsters ._default_titansoul_state ())
    ...
    save_player (username ,root )
    return {
    "success":True ,"titansoul_id":SFSLong (titansoul_id ),
...
def _find_collectable_mine (player_object ):
    ...
    for structure in island .get ("structures")or []:
        ...
        if last_collection <=0 or now >=last_collection +_mine_cooldown_ms (structure ):
            return structure
    return fallback
...
def start_baking (username ,params ):
    ...
    "food_option_id":food_id ,"food_count":food .get ("food",0 ),
...
def start_rebake (username ,params ):
    root ,player_object =load_player (username )
    structure_id =params .get ("user_structure_id")or params .get ("user_baking_id")or player_object .get ("last_rebake_structure_id",0 )
    food_id =params .get ("food_id")or params .get ("food_option_id")or player_object .get ("last_rebake_food_option_id",0 )
    island =None
    structure =None
    if structure_id :
        island ,structure =_locate (player_object ,structure_id )
    if structure is None or not food_id :
        for candidate_island in player_object .get ("islands")or []:
            ...
            if structure is not None :
                break
    if structure is None or not food_id :
        return action_result (False ,"user_structure_id",structure_id or 0 ,with_properties =True )
    params =dict (params )
...
def speed_up_baking (username ,params ):
    ...
    finished_at =int (time .time ()*1000 )
    remaining =(structure .get ("dish_harmonizer_complete_on",finished_at )or finished_at )-finished_at
    _charge_speedup (player_object ,remaining )
...
"Umo","Uom","Fou","Zeb","Kip","Lom","Naf","Sio","Reo","Taj",
]
...
def _rare_id_for (common_id ):
    ...
    if _rare_map_cache is None :
        ...
        _rare_map_cache ={r ["common_id"]:r ["rare_id"]for r in data .get ("rare_monster_data")or []if r .get ("can_rarify")}
...
def _grant_num (num_genes ):
    if num_genes >=4 :
        return get_user_game_setting_int ("USER_DISH_HARMONIZER_TARGET_GRANT_NUM_PRIMORDIAL",28 )
    return get_user_game_setting_int (f"USER_DISH_HARMONIZER_TARGET_GRANT_NUM_{num_genes }_GENE",6 if num_genes ==2 else 9 )
def _pool_for (definition ,num_genes ):
    return [m for m in definition .get ("primordial_requirements")or []if len (_monster_genes (m ))==num_genes ]
...
def ensure_dish_harmonizer_targets (island ,structure ):
    definition =get_ethereal_islet_definition (island .get ("island",0 )if island is not None else 0 )or {}
...
def _target_list (structure ):
    state =_target_state (structure )
    return [{"c":int (state [str (g )].get ("c",0 )or 0 ),"t":state [str (g )]["t"],"g":g }
    for g in (2 ,3 )if isinstance (state .get (str (g )),dict )and state [str (g )].get ("t")]
...
def _pick_results (definition ,structure ):
    ...
    state =_target_state (structure )
    pool =[m for m in definition .get ("primordial_requirements")or []if len (_monster_genes (m ))in (2 ,3 )]
    if not pool :
        return []
    chosen =[]
    for g in (2 ,3 ):
        entry =state .get (str (g ))
        if isinstance (entry ,dict )and int (entry .get ("c",0 )or 0 )>=_grant_num (g )and entry .get ("t")not in chosen :
    ...
    for i ,base in enumerate (chosen ):
        ...
        results .append ({"t":species ,"_b":base ,"_c":[{"g":ch ,"n":n }for ch ,n in counts .items ()],
        "_i":i ,"_n":random .choice (_DISH_HARMONIZER_NICKNAMES )})
    return results
...
def dish_harmonizer_wire_data (player_object ,structure ):
    ...
    "time_required":max (0 ,complete_on -started_on )if pending and started_on else 0 ,
    "targets":targets ,
...
def _clear_dish_harmonizer_state (structure ):
    for key in ("dish_harmonizer_targets","dish_harmonizer_claimed","dish_harmonizer_started_on",
    "dish_harmonizer_complete_on","dish_harmonizer_hits"):
        structure .pop (key ,None )
def _dish_reply (player_object ,structure ,structure_id ):
    data =dish_harmonizer_wire_data (player_object ,structure )
    if data is None :
        data ={"genes":_dish_harmonizer_gene_inventory (player_object ),"started_on":SFSLong (0 ),
        "complete_on":SFSLong (0 ),"time_required":0 ,"targets":[],"structure":SFSLong (structure_id )}
    return data
def start_dish_harmonizing (username ,params ):
    ...
    used_monster =None
    if used_monster_id :
        used_island ,used_monster =find_monster_with_island (player_object ,used_monster_id ,island )
        if used_monster is None :
            return action_result (False ,"user_structure_id",structure_id ,with_properties =True )
    used_definition =get_monster_definition (used_monster .get ("monster",0 ))if used_monster is not None else None
    used_genes =used_definition .get ("genes")if used_definition is not None else ""
    num_genes =len (genes )if isinstance (genes ,list )and genes else len (used_genes )if used_genes else int (params .get ("num_genes",2 )or 2 )
    ...
    if msm_toggles .is_enabled ("functioning_currencies"):
        player_object ["ethereal_currency"]=max (0 ,(player_object .get ("ethereal_currency",0 )or 0 )-cost )
    ...
    save_player (username ,root )
    result =action_result (True ,"user_structure_id",structure_id )
    result ["user_dish_harmonizer_data"]=_dish_reply (player_object ,structure ,structure_id )
    if used_monster_id :
...
def incubate_dish_harmonizer_egg (username ,params ):
    structure_id =params .get ("user_structure_id",0 )
    egg_structure_id =params .get ("egg_structure_id",params .get ("structure_id",0 ))or 0
    root ,player_object =load_player (username )
    island ,structure =_locate (player_object ,structure_id )
    if structure is None :
    ...
    if monster_result_id is not None :
        chosen =next ((t for t in results if t .get ("_i")==monster_result_id and t .get ("_i")not in claimed ),None )
    if chosen is None :
    ...
    if monster_id and island is not None :
        ...
        user_egg ,holder =place_egg (player_object ,island ,monster_id ,"dish_harmonizer",
        egg_structure_id ,previous_name =nickname ,ready =True )
        if user_egg is None :
            structure ["dish_harmonizer_claimed"]=[v for v in claimed if chosen is None or v !=chosen .get ("_i")]
            return {"success":False ,"error":"NO_AVAILABLE_EGG_HOLDER"},_structure_update (structure ,"full")
    if chosen is not None and chosen .get ("_b"):
        ...
        if isinstance (entry ,dict )and entry .get ("t")==base :
            ...
            state [str (g )]={"t":random .choice (alternatives )if alternatives else base ,"c":0 }
    if results and len (claimed )>=len (results ):
        _clear_dish_harmonizer_state (structure )
    save_player (username ,root )
    result ={"success":True ,"user_structure_id":SFSLong (structure_id ),
    "user_dish_harmonizer_data":_dish_reply (player_object ,structure ,structure_id )}
...
def finish_dish_harmonizing (username ,params ):
    ...
    if monster_id and island is not None :
        from msm_monsters import place_egg
        user_egg ,_holder =place_egg (player_object ,island ,monster_id ,"dish_harmonizer_finish",
        structure .get ("user_structure_id",0 ),ready =True )
        if user_egg is None :
            return {"success":False ,"error":"NO_AVAILABLE_EGG_HOLDER"},_structure_update (structure ,"full")
    save_player (username ,root )
    result =action_result (True ,"user_structure_id",structure_id )
