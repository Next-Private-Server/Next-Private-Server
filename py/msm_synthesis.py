...
from msm_gamedata import (
    _user_game_settings,
    get_gene_instability,
    get_monster_id_for_genes,
    get_monster_definition,
    get_structure_definition,
    get_user_game_setting_float,
)
...
def _synthesis_success_chance(genes, structure=None):
    ...
    max_instability = float(
        (definition.get("extra") or {}).get("max_instability", 15) or 15
    )
...
def _charge_synthesis_cost(player_object, genes):
    ...
    player_object["ethereal_currency"] = max(
        0, (player_object.get("ethereal_currency", 0) or 0) - _synthesis_cost(genes)
    )
...
def _used_monster_genes(island, used_monster):
    ...
    definition = (
        get_monster_definition((monster or {}).get("monster", 0)) if monster else None
    )
...
def _consume_critters(island, genes):
    ...
    for used in _used_critters_for_genes(genes):
        for entry in bank:
            ...
            break
    island["attuned_critters"] = [
        e for e in bank if e is not None and (e.get("num", 0) or 0) > 0
    ]
...
def start_synthesizing(username, params):
    ...
    used_monster = (
        params.get("used_monster", 0) or params.get("user_monster_id", 0) or 0
    )
    ...
    combined = _normalized_genes(
        {"genes": _used_monster_genes(island, used_monster) + genes}
    )
    ...
    if succeeded:
        ...
        user_egg = {
            "monster": monster_id,
            "laid_on": SFSLong(now),
            "hatches_on": SFSLong(complete_on),
            "island": SFSLong(island_uid),
            "user_egg_id": SFSLong(_next_egg_id(player_object)),
            "costume": {"p": [], "eq": 0},
            "structure": SFSLong(structure_id),
        }
