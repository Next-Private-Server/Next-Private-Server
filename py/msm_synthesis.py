import random
import time
from msm_gamedata import get_gene_instability ,get_monster_id_for_genes
...
def _synthesis_success_chance (genes ):
    if not genes :
        return 0.5
    instabilities =[get_gene_instability (g )for g in genes ]
    avg =sum (instabilities )/len (instabilities )
    return max (0.15 ,min (0.95 ,1.15 -avg *0.15 ))
...
def start_synthesizing (username ,params ):
    ...
    used_monster =params .get ("used_monster",0 )or 0
    ...
    monster_id =get_monster_id_for_genes (genes )
    succeeded =random .random ()<_synthesis_success_chance (genes )
    complete_on =now +_synthesis_duration_ms (genes ,succeeded )
    synthesis ={
...
def collect_synthesizing_failure (username ,params ):
    ...
    if island is not None :
        ...
        for i in range (len (entries )-1 ,-1 ,-1 ):
            entry =entries [i ]
            if entry is not None and entry .get ("structure")==structure_id :
                _refund_used_critters (island ,entry .get ("used_critters"))
                del entries [i ]
                break
        reattuned =list (island .get ("attuned_critters")or [])
        save_player (username ,root )
    return {"success":True ,"user_structure_id":SFSLong (structure_id ),"reattuned_critters":reattuned }
